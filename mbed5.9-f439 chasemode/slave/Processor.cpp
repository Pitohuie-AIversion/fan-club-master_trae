			...
			*/
			...
			switch(splitPtr[0]){
				...
				case CHASE: // Set a target RPM
				{
					...
					// Parse CHASE command: "CHASE <fanID> <targetRPM>"
					int fanID = -1;
					int targetRPM = -1;
					
					if (sscanf(splitPtr, "CHASE %d %d", &fanID, &targetRPM) == 2) {
						// Validate fan ID
						if (fanID >= 0 && fanID < this->activeFans) {
							
							this->arrayLock.lock();
							
							// Set target RPM for the specified fan
							bool success_chase = this->fanArray[fanID].setTarget(targetRPM);
							
							this->arrayLock.unlock();
							
							if (success_chase) {
								pl;printf(
									"\n\r[%08dms][p] CHASE: Fan %d target set to %d RPM",
									tm, fanID, targetRPM);pu;
								success = true;
							} else {
								pl;printf(
									"\n\r[%08dms][p] ERROR: Failed to set target for fan %d",
									tm, fanID);pu;
								success = false;
							}
							...
							pl;printf(
								"\n\r[%08dms][p] ERROR: Invalid fan ID %d (max: %d)",
								tm, fanID, this->activeFans - 1);pu;
							success = false;
						}
						
					} else {
						pl;printf(
							"\n\r[%08dms][p] ERROR: Invalid CHASE command format",
							tm);pu;
						success = false;
					}
				...
				case PISET: // Set PI controller gains
				{
					pl;printf("\n\r[%08dms][p] PISET Received",tm);pu;
					
					// Parse PISET command: "PISET <fanID> <kp> <ki>"
					int fanID = -1;
					float kp = 0.0f;
					float ki = 0.0f;
					
					if (sscanf(splitPtr, "PISET %d %f %f", &fanID, &kp, &ki) == 3) {
						// Validate fan ID
						if (fanID >= 0 && fanID < this->activeFans) {
							
							this->arrayLock.lock();
							
							// Set PI gains for the specified fan
							bool success_piset = this->fanArray[fanID].setPIGains(kp, ki);
							
							this->arrayLock.unlock();
							
							if (success_piset) {
								pl;printf(
									"\n\r[%08dms][p] PISET: Fan %d gains set to kp=%f, ki=%f",
									tm, fanID, kp, ki);pu;
								success = true;
							} else {
								pl;printf(
									"\n\r[%08dms][p] ERROR: Failed to set PI gains for fan %d",
									tm, fanID);pu;
								success = false;
							}
						...
						pl;printf(
							"\n\r[%08dms][p] ERROR: Invalid PISET command format",
							tm);pu;
						success = false;
					}
		...
		if(this->status ==  OFF){
			...
			if(not this->outFlag) {
				...
				for(int i = 0; i < activeFans; i++){
					...
					// Update chase feedback control if fan is chasing
					if (fanPtr->isChasing()) {
						fanPtr->updateChase(this->chaserTolerance);
					}
