...
Fan::Fan(){
	...
	*/
	...
	this->kp = 0.1f;		// Default proportional gain
	this->ki = 0.01f;		// Default integral gain
	this->integral = 0.0f;
	this->lastError = 0.0f;
	this->chasing = false;
} // End Fan constructor
...
bool Fan::setTarget(int targetRPM){
	/* ABOUT: Set target RPM for chase feedback control.
	 * PARAMETERS:
	 * -int targetRPM: target RPM to chase
	 * RETURNS:
...
bool Fan::updateChase(float tolerance){
	/* ABOUT: Update fan duty cycle using PI feedback control.
	 * PARAMETERS:
	 * -float tolerance: acceptable error tolerance for RPM
	 * RETURNS:
	 * -bool: true if within tolerance, false if still adjusting
	 */
	
	if (!this->initialized || !this->chasing || this->target == NO_TARGET) {
		return false;
	}
	
	// Get current RPM (use last read value to avoid blocking)
	int currentRPM = this->pastRead;
	
	// Calculate error
	float error = (float)(this->target - currentRPM);
	...
	// Get time delta for integral calculation
	float dt = this->chaseTimer.read();
	this->chaseTimer.reset();
	this->chaseTimer.start();
	...
	if (this->integral > 100.0f) this->integral = 100.0f;
	if (this->integral < -100.0f) this->integral = -100.0f;
	
	float output = this->kp * error + this->ki * this->integral;
	
	// Convert PI output to duty cycle adjustment
	float currentDC = this->getDC();
	float newDC = currentDC + (output / 1000.0f); // Scale output appropriately
	
	// Clamp duty cycle to valid range
	if (newDC < 0.0f) newDC = 0.0f;
	if (newDC > 1.0f) newDC = 1.0f;
	
	// Apply minimum duty cycle for nonzero RPM
	if (this->target > 0 && newDC < this->minDC) {
		newDC = this->minDC;
	}
...
bool Fan::setPIGains(float kp, float ki){
	/* ABOUT: Set PI controller gains.
	 * PARAMETERS:
...
int Fan::getTarget(){
	/* ABOUT: Get current target RPM.
	 * RETURNS:
	 * -int: current target RPM, or NO_TARGET if not set
	 */
...
bool Fan::isChasing(){
	/* ABOUT: Check if fan is currently in chase mode.
	 * RETURNS:
	 * -bool: true if chasing, false otherwise
	 */
