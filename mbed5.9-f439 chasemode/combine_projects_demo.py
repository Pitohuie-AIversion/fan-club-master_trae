...
def demonstrate_project_combination():
    ...
    if bootloader_config.exists():
        ...
        try:
            with open(bootloader_config, 'r', encoding='utf-8') as f:
                config = json.load(f)
                if 'target_overrides' in config and 'NUCLEO_F439ZI' in config['target_overrides']:
                    target_config = config['target_overrides']['NUCLEO_F439ZI']
    ...
    # 检查 bootloader 二进制文件
    bootloader_bin = bootloader_dir / "BUILD" / "NUCLEO_F439ZI" / "GCC_ARM-RELEASE" / "slave_bootloader.bin"
    ...
    # 检查应用程序二进制文件
    app_bin = application_dir / "BUILD" / "NUCLEO_F439ZI" / "GCC_ARM-RELEASE" / "slave.bin"
    if app_bin.exists():
