...
def setup_event_handlers(system):
    ...
    def on_data_acquired(data):
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] 数据采集: 形状={data['data_shape']}"
        )

    def on_data_filtered(data):
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] 数据滤波: 原始={data['original_shape']} -> 滤波后={data['filtered_shape']}"
        )
    ...
    def on_quality_updated(data):
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] 质量更新 - {channel}: "
    ...
    def on_anomaly_detected(data):
        ...
        channel = data["channel"]
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️  异常检测 - {channel}: "
    ...
    def on_data_saved(data):
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] 💾 {save_type}保存: {filepath} (形状={shape})"
        )
...
def print_system_status(system):
    ...
    print(
        f"  原始数据缓冲区: {buffer_status['raw_data_buffer_size']} (满: {buffer_status['raw_data_buffer_full']})"
    )
    print(
        f"  滤波数据缓冲区: {buffer_status['filtered_data_buffer_size']} (满: {buffer_status['filtered_data_buffer_full']})"
    )
    ...
    if quality_summary:
        ...
        for channel, summary in quality_summary.items():
            if isinstance(summary, dict):
                ...
                print(
                    f"  质量等级分布: {summary.get('quality_level_distribution', {})}"
                )
