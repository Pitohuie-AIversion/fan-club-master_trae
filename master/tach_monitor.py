...
class TachSignalMonitor:
    ...
    def __init__(self, max_slaves=8, max_fans_per_slave=16, fc_communicator=None):
        ...
        self.stats = {
            ...
            "performance": {"avg_read_time": 0, "avg_process_time": 0, "data_rate": 0},
        }
        ...
        while self.is_monitoring:
            try:
                ...
                if consecutive_errors >= self.max_consecutive_errors:
                    print(
                        f"连续错误次数超过限制({self.max_consecutive_errors})，保持非模拟模式"
                    )
                ...
                # 错误恢复延迟
                recovery_delay = min(
                    self.error_recovery_delay * consecutive_errors, 10.0
                )
    ...
    def _check_connection_health(self):
        ...
        if current_time - self.last_successful_read > self.data_timeout_threshold:
            ...
            raise Exception(
                f"数据读取超时 ({current_time - self.last_successful_read:.1f}s)"
            )
        ...
        if (
            hasattr(self.fc_communicator, "stopped")
            and self.fc_communicator.stopped.is_set()
        ):
    ...
    def _read_real_data(self):
        ...
        try:
            ...
            if self.fc_communicator is not None and hasattr(
                self.fc_communicator, "get_rpm_data"
            ):
    ...
    def _update_data_cache(self, rpm_data, timestamp):
        ...
        self.data_cache = {
            "rpm_data": (
                rpm_data.copy() if hasattr(rpm_data, "copy") else list(rpm_data)
            ),
            "timestamp": timestamp,
        }
    ...
    def _handle_no_data_warning(self):
        ...
        if self._no_data_warning_count % 100 == 1:
            print(
                f"警告: FCCommunicator未返回RPM数据 (第{self._no_data_warning_count}次)"
            )
    ...
    def _update_performance_stats(self):
        ...
        if self.performance_stats["read_times"]:
            ...
            self.stats["performance"]["avg_read_time"] = round(
                avg_read_time * 1000, 2
            )  # 转换为毫秒

        if self.performance_stats["process_times"]:
            avg_process_time = np.mean(list(self.performance_stats["process_times"]))
            self.stats["performance"]["avg_process_time"] = round(
                avg_process_time * 1000, 2
            )  # 转换为毫秒

        # 计算数据率（每秒处理的数据点数）
        current_time = time.time()
        recent_data = [
            data for data in self.signal_data if current_time - data.timestamp <= 1.0
        ]
        ...
        try:
            ...
            if isinstance(rpm_data, dict):
                ...
                for fan_key, rpm_value in rpm_data.items():
                    ...
                    # 验证单个RPM值
                    validated_rpm = self._validate_single_rpm(
                        rpm_value, slave_id, fan_id
                    )
    ...
    def _is_rpm_outlier(self, rpm_value, slave_id, fan_id):
        ...
        recent_data = [
            data
            for data in list(self.signal_data)[-20:]
    ...
    def _smooth_rpm_value(self, rpm_value, slave_id, fan_id):
        ...
        recent_data = [
            data
            for data in list(self.signal_data)[-5:]
            if data.slave_id == slave_id and data.fan_id == fan_id
        ]
    ...
    def _calculate_signal_quality(self, slave_id, fan_id, current_rpm):
        ...
        try:
            ...
            recent_data = [
                data
                for data in list(self.signal_data)[-10:]
                if data.slave_id == slave_id and data.fan_id == fan_id
            ]
    ...
    def _update_stats(self, data_point: TachSignalData):
        ...
        if slave_fan_key not in self.stats["rpm_ranges"]:
            self.stats["rpm_ranges"][slave_fan_key] = {
                "min": data_point.rpm,
                "max": data_point.rpm,
            }
        else:
    ...
    def save_data(self, filename: str):
        ...
        data_to_save = {
            "metadata": {
                ...
                - (self.signal_data[0].timestamp if self.signal_data else time.time()),
    ...
    def print_status(self):
        ...
        print(
            f"最后更新: {time.strftime('%H:%M:%S', time.localtime(stats['last_update']))}"
        )

        print("\n风机详情:")
        for fan_key, details in stats["fan_details"].items():
            print(
                f"  {fan_key}: RPM范围 {details['rpm_min']}-{details['rpm_max']}, "
