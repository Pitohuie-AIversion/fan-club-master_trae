...
class TachSignalDiagnostics:
    ...
    def _check_no_signal(
        self, fan_id: str, readings: List[Dict]
    ) -> Optional[TachDiagnosticResult]:
        """检查无信号问题"""
        if not readings:
    ...
    def _check_timeout_issues(
        ...
        if timeout_rate > self.thresholds["max_timeout_rate"]:
            return TachDiagnosticResult(
                ...
                data_points=[r for r in readings if r.get("timeout_occurred", False)][
                    -5:
                ],
            )
    ...
    def _check_pulse_count_anomaly(
        self, fan_id: str, readings: List[Dict]
    ) -> Optional[TachDiagnosticResult]:
        """检查脉冲计数异常"""
...
def main():
    ...
    # 查找数据文件
    data_files = [
        f for f in os.listdir(".") if f.startswith("tach_") and f.endswith(".json")
    ]
