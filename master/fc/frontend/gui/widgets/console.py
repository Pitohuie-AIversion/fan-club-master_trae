...
class ConsoleWidget(ttk.Frame):
    ...
    def __init__(self, master, warnMethod):
        ...
        self.screen.bind("<1>", lambda event: self.screen.focus_set())
        ...
        self.screen.tag_config(TAG_ERROR, foreground=FG_ERROR, background=BG_ERROR)
        ...
        self.controlFrame.grid(row=2, column=0, columnspan=2, sticky="EW")

        # Save log button:
        self.saveButton = ttk.Button(
            self.controlFrame, text="Save Log", command=self._save
        )
        ...
        self.debugVar.set(0)
        self.debugButton = ttk.Checkbutton(
            self.controlFrame,
            text="Debug prints",
        ...
        self.autoscrollButton = ttk.Checkbutton(
            self.controlFrame, text="Autoscroll", variable=self.autoscrollVar
        )
        ...
        try:
            self.maxLinesSpin = ttk.Spinbox(
                self.controlFrame,
                from_=500,
                to=200000,
                increment=500,
                width=7,
                textvariable=self.maxLinesVar,
            )
    ...
    def _print(self, tag, text):
        ...
        try:
            ...
            if (
                ...
                or not hasattr(self, "screen")
                or not self.screen.winfo_exists()
            ):
                return
        ...
        try:
            ...
            if not filename:
                ...
                with open(filename, "w") as f:
                    f.write(
                        "Fan Club MkIV Terminal log printed on {}\n\n".format(
            ...
            gus.popup_exception(
                "FCMKIV Error", "Exception in Terminal print-to-file", e
            )
    ...
    def _copy(self, *E):
        try:
            ...
            gus.popup_exception(
                "FCMKIV Error", "Exception in Console copy-to-clipboard", e
            )
