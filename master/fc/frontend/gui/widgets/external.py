...
class ExternalControlWidget(pt.PrintClient, ttk.Frame):
    ...
    def __init__(self, master, archive, backend, pqueue):
        ...
        self.startAllButton = ttk.Button(
            self.allFrame, text="Start All", command=self._onStartAll
        )
        self.startAllButton.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.stopAllButton = ttk.Button(
            self.allFrame, text="Stop All", command=self._onStopAll
        )
        self.stopAllButton.pack(side=tk.LEFT, fill=tk.X, expand=True)
        row += 1

        self.broadcast = ECSetupWidget(
            self,
            "State Broadcast",
    ...
    def set(self, key, value, backend=True):
        try:
            ...
            self.printx(
                e,
                "Exception while {}activating {}".format(
    ...
    def _startListenerBackEnd(self):
        self.backend.activateListener(
            port=self.listener.getPort(), repeat=self.listener.getRepeat()
        )
...
class ECSetupWidget(ttk.Frame):
    ...
    def __init__(
        ...
        _, self.inputIndexEntry = self._entryPair(
            self.inputIndexFrame, self.PARAM_REPEAT, "In: ", validateN, active=False
        )
        self.inputIndexVar = tk.IntVar()
        self.inputIndexEntry.config(state=tk.DISABLED, textvariable=self.inputIndexVar)
        self.outputIndexFrame = self._gridFrame(self.main, low, left)
        _, self.outputIndexEntry = self._entryPair(
            self.outputIndexFrame, self.PARAM_REPEAT, "Out: ", validateN, active=False
        )
        self.outputIndexVar = tk.IntVar()
        self.outputIndexEntry.config(
            state=tk.DISABLED, textvariable=self.outputIndexVar
        )

        # IP and Port:
        self.ipFrame = self._gridFrame(self.main, top, right)
        _, self.ipEntry = self._entryPair(
            self.ipFrame, self.PARAM_IP, "IP: ", validateA, self.editIP, 15
        )
        if not self.editIP:
            self.ipEntry.config(state=tk.DISABLED)
        self.portFrame = self._gridFrame(self.main, mid, right)
        _, self.portEntry = self._entryPair(
            self.portFrame, self.PARAM_PORT, "Port: ", validateN
        )

        # Index and Repeat:
        self.repeatFrame = self._gridFrame(self.main, low, right)
        _, self.repeatEntry = self._entryPair(
            self.repeatFrame, self.PARAM_REPEAT, "Repeat: ", validateN
        )

        # Start and stop button:
        self.startStopFrame = self._gridFrame(self.main, bottom, left)
        self.startStopButton = ttk.Button(
            self.startStopFrame, text="Start", command=self._onStart
        )
    ...
    def _gridFrame(self, master, row, column, columnspan=1):
        ...
        frame.grid(row=row, column=column, sticky="NEWS", columnspan=columnspan)
        return frame
    ...
    def _entryPair(self, master, key, text, validator, active=True, width=7):
        ...
        entry = ttk.Entry(
            master, validate="key", validatecommand=(validator, "%S", "%s", "%d")
        )
