...
from fc.frontend.gui.animations import (
    AnimationManager,
...
class Base(ttk.Frame, ResponsiveMixin):
    ...
    ERROR_MESSAGE = "[There are error messages in the console. Click here.]"
    ...
    def __init__(
        ...
        try:
            ...
            self.brandLabel = ttk.Label(
                self.topBar, image=self.brandImage, style="Topbar.TLabel"
            )
            ...
            try:
                ...
                pass
            self.brandLabel = ttk.Label(
                self.topBar, text="ZHAOYANG · DASHUAI", style="Topbar.TLabel"
            )
            self.brandLabel.pack(side=tk.LEFT, ipady=0, pady=(0, 0), padx=10)
        self.errorLabel = ttk.Label(
            self.topBar, text=self.ERROR_MESSAGE, style="ErrorBanner.TLabel"
        )
        ...
        self.settingsButton = ttk.Button(
            self.topBar,
            text=" Settings ",
            command=self._settingsCallback,
        ...
        ttk.Label(self.profileCard, text="Profile", style="TitleLabel.TLabel").pack(
            anchor=tk.W, pady=(0, 8)
        )
        self.profileWidget = pro.ProfileDisplay(
            self.profileCard, archive, profileCallback, pqueue
        )
        ...
        ttk.Label(self.networkCard, text="Network", style="TitleLabel.TLabel").pack(
            anchor=tk.W, pady=(0, 8)
        )
        self.networkWidget = ntw.NetworkWidget(
            self.networkCard,
            network=network,
            archive=archive,
            networkAdd=self.networkAdd,
        ...
        ttk.Label(self.controlCard, text="Control", style="TitleLabel.TLabel").pack(
            anchor=tk.W, pady=(0, 8)
        )
        self.controlWidget = ctr.ControlWidget(
            self.controlCard,
            network=network,
            external=external,
            mapper=mapper,
        ...
        ttk.Label(self.consoleCard, text="Console", style="TitleLabel.TLabel").pack(
            anchor=tk.W, pady=(0, 8)
        )
        self.consoleWidget = csl.ConsoleWidget(self.consoleCard, self._consoleWarning)
        ...
        ttk.Label(
            self.monitoringCard, text="Data Monitoring", style="TitleLabel.TLabel"
        ).pack(anchor=tk.W, pady=(0, 8))
        self.monitoringWidget = mon.MonitoringWidget(
            self.monitoringCard, archive=archive, pqueue=pqueue
        )
        ...
        ttk.Label(
            self.filterCard, text="Filter Configuration", style="TitleLabel.TLabel"
        ).pack(anchor=tk.W, pady=(0, 8))
        self.filterWidget = flt.FilterConfigWidget(
            self.filterCard,
            archive=archive,
            pqueue=pqueue,
            monitoring_widget=self.monitoringWidget,
        )
        ...
        self.bottomWidget = ntw.StatusBarWidget(
            self.bottomBar, network.shutdown, pqueue
        )
        self.bottomWidget.pack(side=tk.LEFT, fill=tk.X, expand=True, pady=6, padx=8)
    ...
    def _show_settings_menu(self):
        ...
        title_label = ttk.Label(
            title_frame, text="⚙️ Settings", font=("Segoe UI", 14, "bold")
        )
        ...
        try:
            ...
            print(
                f"[DEBUG] Settings window - Current theme from manager: {current_theme}"
            )
        except Exception as e:
            current_theme = "Light"
            print(
                f"[DEBUG] Settings window - Error getting theme, defaulting to Light: {e}"
            )
        ...
        theme_value_label = ttk.Label(
            theme_info_frame, text=current_theme, font=("Segoe UI", 9, "bold")
        )
        ...
        print(
            f"[DEBUG] Settings window - Theme variable initialized to: {theme_var.get()}"
        )
        theme_radio_frame = ttk.Frame(theme_section)
        theme_radio_frame.pack(fill=tk.X, padx=10, pady=(0, 10))

        light_radio = ttk.Radiobutton(
            theme_radio_frame, text="Light Theme", variable=theme_var, value="light"
        )
        light_radio.pack(anchor=tk.W, pady=2)

        dark_radio = ttk.Radiobutton(
            theme_radio_frame, text="Dark Theme", variable=theme_var, value="dark"
        )
        ...
        density_value_label = ttk.Label(
            density_info_frame,
            text=current_density.title(),
        ...
        comfortable_radio = ttk.Radiobutton(
            density_radio_frame,
            text="Comfortable (Recommended)",
            variable=density_var,
            value="comfortable",
        )
        comfortable_radio.pack(anchor=tk.W, pady=2)

        compact_radio = ttk.Radiobutton(
            density_radio_frame, text="Compact", variable=density_var, value="compact"
        )
        ...
        hardware_status_label = ttk.Label(
            status_frame,
        ...
        simulated_radio = ttk.Radiobutton(
            hardware_radio_frame,
        ...
        def on_hardware_change():
            ...
            if new_hardware_type != current_hardware_type and not hardware_running:
                try:
                    if hasattr(self, "acquisition_engine"):
                        success = self.acquisition_engine.switch_hardware_type(
                            new_hardware_type
                        )
                        if success:
                            # Update status display
                            hardware_status_label.config(
                                text=f"{new_hardware_type.title()} Hardware"
                            )
                            ...
                            if new_hardware_type == "real":
                                ...
                                status_text = (
                                    "Connected" if new_connected else "Disconnected"
                                )
                            else:
                                status_color = "🟡"
                                status_text = "Simulated"
                            ...
                            status_indicator.config(
                                text=f"{status_color} {status_text}"
                            )
        ...
        if hardware_running:
            ...
            ttk.Label(warning_frame, text=info_text, foreground="blue").pack(
                anchor=tk.W
            )
        ...
        ttk.Label(info_frame, text="Fan Club MkIV", font=("Segoe UI", 10, "bold")).pack(
            anchor=tk.W
        )
        ttk.Label(info_frame, text="Version: 4.0").pack(anchor=tk.W, pady=(5, 0))
        ttk.Label(info_frame, text="Enhanced UI with modern features").pack(
        ...
        def apply_settings():
            try:
                ...
                if new_theme != current_theme.lower():
                    print(
                        f"[DEBUG] Applying theme change from {current_theme} to {new_theme}"
                    )
        ...
        ttk.Button(
            button_frame,
            text="OK",
            command=lambda: (apply_settings(), settings_window.destroy()),
    ...
    def _toggle_theme_and_close(self, window):
        ...
        def complete_toggle():
            ...
            self.performance_manager.optimized_theme_switch(
                theme_manager,
        ...
        animate_color_transition(
            window, duration=300, easing=ease_in_out_cubic, callback=complete_toggle
        )
    ...
    def _toggle_density_and_close(self, window):
        ...
        def complete_toggle():
            current_density = getattr(self, "_ui_density", "comfortable")
            new_density = (
                "compact" if current_density == "comfortable" else "comfortable"
            )
            self._ui_density = new_density
            self._apply_density_settings(new_density)
            window.destroy()
        ...
        animate_color_transition(
            window, duration=200, easing=ease_in_out_cubic, callback=complete_toggle
        )
    ...
    def _apply_density_settings(self, density):
        ...
        try:
            if (
                hasattr(self, "master")
                and self.master
                and hasattr(self.master, "winfo_exists")
                ...
                if density == "compact":
                    ...
                    style.configure(
                        "Secondary.TButton", padding=(6, 3), font=("Segoe UI", 8)
                    )
                    ...
                    style.configure(
                        "TLabelFrame", padding=4, font=("Segoe UI", 8, "bold")
                    )

                    # Additional controls
                    style.configure(
                        "TCheckbutton", padding=(3, 1), font=("Segoe UI", 8)
                    )
                    style.configure(
                        "TRadiobutton", padding=(3, 1), font=("Segoe UI", 8)
                    )
                    ...
                    # Notebook tabs
                    style.configure(
                        "TNotebook.Tab", padding=(8, 4), font=("Segoe UI", 8)
                    )

                    # Special styles
                    style.configure(
                        "Topbar.TLabel", padding=(3, 1), font=("Segoe UI", 8)
                    )
                    style.configure(
                        "TitleLabel.TLabel",
                        padding=(3, 2),
                    ...
                    style.configure(
                        "Secondary.TButton", padding=(10, 6), font=("Segoe UI", 9)
                    )
                    ...
                    style.configure(
                        "TLabelFrame", padding=8, font=("Segoe UI", 9, "bold")
                    )

                    # Additional controls
                    style.configure(
                        "TCheckbutton", padding=(6, 4), font=("Segoe UI", 9)
                    )
                    style.configure(
                        "TRadiobutton", padding=(6, 4), font=("Segoe UI", 9)
                    )
                    ...
                    # Notebook tabs
                    style.configure(
                        "TNotebook.Tab", padding=(12, 8), font=("Segoe UI", 9)
                    )

                    # Special styles
                    style.configure(
                        "Topbar.TLabel", padding=(6, 4), font=("Segoe UI", 9)
                    )
                    style.configure(
                        "TitleLabel.TLabel",
                        padding=(6, 4),
    ...
    def _switch_tab_shortcut(self, tab_index: int):
        ...
        """
        try:
            if hasattr(self, "notebook") and 0 <= tab_index < self.notebook.index(
                "end"
            ):
                ...
                for i, tooltip_text in enumerate(tab_tooltips):
                    try:
                        ...
                        if i == 0:  # Only add for the first tab to avoid duplication
                            self.ux_manager.add_tooltip(
                                self.notebook, "Use Ctrl+1-4 to quickly switch tabs"
                            )
