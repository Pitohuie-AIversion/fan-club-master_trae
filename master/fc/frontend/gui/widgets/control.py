...
from fc.frontend.gui.widgets import (
    grid as gd,
    loader as ldr,
    timer as tmr,
    external as ex,
...
class ControlWidget(ttk.Frame, pt.PrintClient):
    ...
    def __init__(
        self, master, network, external, mapper, archive, pqueue, setLiveBE, setFBE
    ):
    ...
    def _buildControl(self):
        ...
        if self.control is not None:
            ...
            self.control = None
        self.control = ControlPanelWidget(
            self.controlFrame,
            self.mapper,
            self.archive,
            self.network,
    ...
    def _buildDisplays(self):
        ...
        self.grid = GridWidget(
            self.display, self.archive, self.mapper, self._send, pqueue=self.pqueue
        )
...
class PythonInputWidget(ttk.Frame):
    ...
    def __init__(self, master, callback, pqueue):
        ...
        self.topLabel = ttk.Label(
            self, text=self.signature, anchor=tk.W, style="Secondary.TLabel"
        )
        ...
        self.scrollbar.grid(row=row, column=2, rowspan=1, sticky="NS")
        ...
        # TODO
        self.runButton = ib.create_apply_button(self.buttonFrame, command=self._run)
    ...
    def _parse(self):
        ...
        for keyword in dangerous_keywords:
            if keyword in raw.lower():
                self.printx(
                    f"Security warning: '{keyword}' is not allowed in user code"
                )
                return None
        ...
        try:
            ...
            def exec_with_timeout():
                try:
                    ...
                    result_queue.put(e)  # 捕获异常

            # 创建结果队列和线程
            result_queue = queue.Queue()
            exec_thread = threading.Thread(target=exec_with_timeout)
            ...
            try:
                ...
                if result is not None:
                    raise result  # 重新抛出执行中的异常
            except queue.Empty:
                raise TimeoutError("代码执行超时（限制5秒）")
            finally:
                # 确保线程不会无限运行
                if exec_thread.is_alive():
                    # 注意：无法强制终止Python线程，但daemon线程会在主线程结束时自动终止
                    pass
...
class MainControlWidget(ttk.Frame, pt.PrintClient):
    ...
    def __init__(self, master, network, display, logstart, logstop, pqueue):
        ...
        # Direct input .........................................................
        self.directFrame = ttk.LabelFrame(self.simpleFrame, text="Direct input")
        ...
        self.directValueEntry = ttk.Entry(
            self.directFrame, width=6, validate="key", validatecommand=vcmd
        )
        self.directValueEntry.pack(side=tk.LEFT, **gus.padc)
        # Input validation implemented for duty cycle values (0-100)
        self.sendDirectButton = ttk.Button(
            self.directFrame, text="Apply", command=self._onSendDirect
        )
        ...
        self.randomLow = ttk.Entry(
            self.randomFrame, width=5, validate="key", validatecommand=vcmd
        )
        ...
        self.sendRandomButton = ttk.Button(
            self.randomFrame, text="Apply", command=self._sendRandom
        )
        ...
        self.chaseLabel = ttk.Label(
            self.chaseControlFrame, text="Target RPM:", style="Secondary.TLabel"
        )
        ...
        self.chaseFanLabel = ttk.Label(
            self.chaseFanFrame, text="Fan Selection:", style="Secondary.TLabel"
        )
        self.chaseFanLabel.pack(side=tk.LEFT, **gus.padc)

        # Target selection radio buttons
        self.chaseTargetVar = tk.StringVar(value="all")
        self.chaseAllRadio = ttk.Radiobutton(
            self.chaseFanFrame,
            text="All Fans",
            variable=self.chaseTargetVar,
            value="all",
            command=self._onChaseTargetChange,
        )
        self.chaseAllRadio.pack(side=tk.LEFT, **gus.padc)
        self.activeWidgets.append(self.chaseAllRadio)

        self.chaseSelectedRadio = ttk.Radiobutton(
            self.chaseFanFrame,
            text="Selected",
            variable=self.chaseTargetVar,
            value="selected",
        ...
        self.chaseFanSelectionLabel = ttk.Label(
            self.chaseFanSelectionFrame,
        ...
        self.chaseHelpLabel = ttk.Label(
            self.chaseFanSelectionFrame,
            text="(1=selected, 0=not selected)",
            style="Secondary.TLabel",
        )
        ...
        self.chaseStartButton = ttk.Button(
            self.chaseButtonFrame, text="Start CHASE", command=self._onStartChase
        )
        self.chaseStartButton.pack(side=tk.LEFT, **gus.padc)
        self.activeWidgets.append(self.chaseStartButton)

        self.chaseStopButton = ttk.Button(
            self.chaseButtonFrame,
            text="Stop",
            command=self._onStopChase,
        ...
        self.chaseStatusLabel = ttk.Label(
            self.chaseFrame, text="Status: Ready", style="Secondary.TLabel"
        )
        ...
        self.kpLabel = ttk.Label(
            self.kpFrame, text="Kp (比例增益):", style="Secondary.TLabel"
        )
        ...
        self.kpEntry = ttk.Entry(
            self.kpFrame, width=8, validate="key", validatecommand=vcmd_kp
        )
        ...
        self.kpScale = ttk.Scale(
            self.kpFrame,
            from_=0.1,
            to=2.0,
            command=self._onKpScaleChange,
            orient=tk.HORIZONTAL,
            length=150,
        )
        ...
        self.kpRangeLabel = ttk.Label(
            self.kpFrame, text="(0.1-2.0)", style="Secondary.TLabel"
        )
        ...
        self.kiLabel = ttk.Label(
            self.kiFrame, text="Ki (积分增益):", style="Secondary.TLabel"
        )
        ...
        self.kiEntry = ttk.Entry(
            self.kiFrame, width=8, validate="key", validatecommand=vcmd_ki
        )
        ...
        self.kiScale = ttk.Scale(
            self.kiFrame,
            from_=0.01,
            to=0.5,
            command=self._onKiScaleChange,
            orient=tk.HORIZONTAL,
            length=150,
        )
        ...
        self.kiRangeLabel = ttk.Label(
            self.kiFrame, text="(0.01-0.5)", style="Secondary.TLabel"
        )
        ...
        self.autoTuneButton = ttk.Button(
            self.piButtonFrame, text="自动调优", command=self._onPIAutoTune
        )
        self.autoTuneButton.pack(side=tk.LEFT, **gus.padc)
        self.activeWidgets.append(self.autoTuneButton)

        self.resetPIButton = ttk.Button(
            self.piButtonFrame,
            text="重置默认值",
            command=self._onPIReset,
            style="Secondary.TButton",
        )
        self.resetPIButton.pack(side=tk.LEFT, **gus.padc)
        self.activeWidgets.append(self.resetPIButton)

        self.applyPIButton = ttk.Button(
            self.piButtonFrame, text="应用参数", command=self._onPIApply
        )
        ...
        self.piCurrentLabel = ttk.Label(
            self.piStatusFrame,
            text="当前参数: Kp=0.01, Ki=0.001",
            style="Secondary.TLabel",
        )
        self.piCurrentLabel.pack(side=tk.LEFT, **gus.padc)

        self.piPerformanceLabel = ttk.Label(
            self.piStatusFrame, text="性能: 稳定", style="Secondary.TLabel"
        )
        self.piPerformanceLabel.pack(side=tk.RIGHT, **gus.padc)

        # Status message display (separate from current parameters)
        self.piStatusLabel = ttk.Label(
            self.piStatusFrame, text="状态: 就绪", style="Secondary.TLabel"
        )
        ...
        self.flowLoader = ldr.FlowLoaderWidget(self.flowLoaderFrame, self._onLoad)
        self.flowLoader.pack(side=tk.LEFT)
        self.activeWidgets.append(self.flowLoader)

        self.fileLabel = ttk.Label(
            self.flowLoaderFrame, text="File: ", style="Secondary.TLabel"
        )
        self.fileLabel.pack(side=tk.LEFT)
        self.fileField = ttk.Entry(self.flowLoaderFrame, width=20, state=tk.DISABLED)
        self.fileField.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.fileTypeLabel = ttk.Label(
            self.flowLoaderFrame, text="Type: ", style="Secondary.TLabel"
        )
        ...
        self.typeMenu = ttk.OptionMenu(
            self.flowLoaderFrame,
            self.typeMenuVar,
            self.typeMenuVar.get(),
    ...
    def _sendRandom(self, *_):
        ...
        """
        try:
            minDC, maxDC = float(self.randomLow.get()), float(self.randomHigh.get())
            if maxDC < minDC:
                raise ValueError(
                    "Upper bound ({maxDC:d}) cannot be "
    ...
    def _onStartChase(self, *_):
        ...
        try:
            ...
            if target_mode == "all":
                ...
                self.chaseStatusLabel.config(
                    text="Status: Feedback CHASE Active - All Fans (Target: {} RPM)".format(
                        int(targetRPM)
                    )
                )
                self.printw(
                    "Started feedback-controlled CHASE mode for all fans with target RPM: {}".format(
                        targetRPM
                    )
                )
            else:
                ...
                self.chaseStatusLabel.config(
                    text="Status: Feedback CHASE Active - {} Selected Fans (Target: {} RPM)".format(
                        selected_count, int(targetRPM)
                    )
                )
                self.printw(
                    "Started feedback-controlled CHASE mode for selected fans ({}) with target RPM: {}".format(
                        selection_string, targetRPM
                    )
                )
    ...
    def _onPIReset(self, *_):
        ...
        try:
            ...
            # Update current parameters display
            self.piCurrentLabel.config(
                text=f"当前参数: Kp={default_kp:.4f}, Ki={default_ki:.4f}"
            )
            ...
            self.printw(
                f"PI参数已重置为默认值: Kp={default_kp:.4f}, Ki={default_ki:.4f}"
            )
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.network, "sendPISet"):
                    ...
                    self.piStatusLabel.config(
                        text="状态: 参数已应用(本地)", fg="orange"
                    )
    ...
    def _onKpScaleChange(self, value):
        ...
        try:
            ...
            # Update current parameters display
            ki = (
                float(self.kiEntry.get())
                if self.kiEntry.get()
                else getattr(self, "ki", 0.001)
            )
            self.piCurrentLabel.config(text=f"当前参数: Kp={kp:.4f}, Ki={ki:.4f}")
    ...
    def _onKiScaleChange(self, value):
        ...
        try:
            ...
            # Update current parameters display
            kp = (
                float(self.kpEntry.get())
                if self.kpEntry.get()
                else getattr(self, "kp", 0.01)
            )
            self.piCurrentLabel.config(text=f"当前参数: Kp={kp:.4f}, Ki={ki:.4f}")
    ...
    def _showPITuning(self, *_):
        ...
        try:
            if hasattr(self, "feedback_active") and self.feedback_active:
                ...
                self.piCurrentLabel.config(
                    text=f"当前参数: Kp={current_kp:.4f}, Ki={current_ki:.4f}"
                )
    ...
    def _startFeedbackChase(
        self, target_rpm, selection_mode="all", selection_string=""
    ):
        """
        ...
        try:
            ...
            if hasattr(self.display, "currentWidget"):
                ...
                if hasattr(current_widget, "mapper") and hasattr(
                    current_widget.mapper, "maxFans"
                ):
            ...
            self.printw(
                "Started feedback-controlled CHASE mode (Target: {} RPM, Kp: {:.4f}, Ki: {:.4f})".format(
    ...
    def _autoTunePI(self, current_error, current_rpm):
        ...
        error_variance = sum((e - avg_error) ** 2 for e in self.error_history) / len(
            self.error_history
        )
        ...
        if avg_error > self.target_rpm * 0.1:  # High steady-state error
            ...
            self.ki *= 0.9
            self.printw(
                f"Auto-tune: Reduced gains - Kp: {self.kp:.4f}, Ki: {self.ki:.4f} (oscillation)"
            )

        elif (
            avg_error < self.target_rpm * 0.02
            and error_variance < (self.target_rpm * 0.02) ** 2
        ):
    ...
    def _antiWindupProtection(self, integral_error, max_integral=None):
        ...
        if protected_integral != integral_error:
            self.printw(
                f"Anti-windup: Integral clamped from {integral_error:.2f} to {protected_integral:.2f}"
            )
    ...
    def _updateFeedbackChase(self):
        ...
        try:
            ...
            current_widget = self.display.currentWidget()
            if (
                not hasattr(current_widget, "feedback")
                or current_widget.feedback is None
            ):
        ...
        for fan_id, current_rpm in enumerate(feedback):
            if current_rpm is not None and current_rpm > 0:
                ...
                if processed_error != 0:  # Only update if outside dead zone
                    ...
                    self.integral_error = self._antiWindupProtection(
                        self.integral_error
                    )

                    # PI control calculation
                    adjustment = (
                        self.kp * processed_error + self.ki * self.integral_error
                    )
    ...
    def _updateSelectedFans(self, feedback):
        ...
        for fan_id, current_rpm in enumerate(feedback):
            ...
            if (
                fan_id < len(self.selection_string)
                and self.selection_string[fan_id] == "1"
            ):
                if current_rpm is not None and current_rpm > 0:
                    # Calculate RPM error
                    rpm_error = self.target_rpm - current_rpm
                    ...
                    # Ensure per-fan integral error dict is always initialized before use
                    # (must be outside the dead-zone guard so the key exists when
                    #  processed_error == 0 and we still need it for the adjustment calc)
                    ...
                    if processed_error != 0:  # Only update integral if outside dead zone
                        self.fan_integral_errors[fan_id] += rpm_error

                        # Apply anti-windup protection
                        self.fan_integral_errors[fan_id] = self._antiWindupProtection(
                            self.fan_integral_errors[fan_id]
                        )

                    # PI control calculation (safe: fan_integral_errors[fan_id] always exists)
                    adjustment = (
                        self.kp * processed_error
                        + self.ki * self.fan_integral_errors[fan_id]
                    )
                    ...
                    current_time = tm.time()
                    if (current_time - self.last_tuning_time) > self.tuning_interval:
                        # Bug fix: pass required arguments (current_error, current_rpm)
                        self._autoTunePI(rpm_error, current_rpm)
                        self.last_tuning_time = current_time
...
class FunctionControlWidget(ttk.Frame, pt.PrintClient):
    ...
    def __init__(self, master, display, logstart, logstop, pqueue):
        ...
        row += 1
        self.python = PythonInputWidget(self.pythonFrame, self._applyFunction, pqueue)
...
class FlowLibraryWidget(ttk.Frame, pt.PrintClient):
    ...
    def __init__(self, master, flows, display, pqueue):
        ...
        self.list.grid(row=LIST_ROW, column=LIST_COLUMN, sticky="NEW")
        # Add columns:
        ...
        self.hscrollbar.grid(row=HSCROLL_ROW, column=HSCROLL_COLUMN, sticky="EW")
        self.vscrollbar = ttk.Scrollbar(self, orient=tk.VERTICAL)
        self.vscrollbar.config(command=self.list.yview)
        self.vscrollbar.grid(row=VSCROLL_ROW, column=VSCROLL_COLUMN, sticky="NS")

        # Build description display
        self.descriptionFrame = ttk.LabelFrame(self, text="About")
        self.descriptionFrame.grid(
            row=DESC_ROW, column=DESC_COLUMN, sticky="NEWS", columnspan=2
        )
        self.descriptionFrame.grid_columnconfigure(0, weight=1)

        self.nameDisplay = ttk.Label(
            self.descriptionFrame, anchor=tk.W, style="TitleLabel.TLabel"
        )
        self.nameDisplay.grid(row=0, column=0, sticky="EW", pady=(0, 6))

        self.descriptionDisplay = tk.Text(
        ...
        self.dscrollbar = ttk.Scrollbar(self.descriptionFrame, orient=tk.VERTICAL)
        ...
        self.attributeFrame.grid(row=ATTR_ROW, column=ATTR_COLUMN, sticky="NEWS")
        ...
        self.ascrollbar.grid(row=ASCROLL_ROW, column=ASCROLL_COLUMN, sticky="NS")

        # Build control
        self._apply = lambda: None  # FIXME
        self.controlFrame = ttk.LabelFrame(self, text="Control")
        self.controlFrame.grid(
            row=CTRL_ROW, column=CTRL_COLUMN, sticky="NEWS", columnspan=2
        )
        self.applyButton = ttk.Button(
            self.controlFrame, text="Apply", command=self._apply
        )
...
class ControlPanelWidget(ttk.Frame, pt.PrintClient):
    ...
    def __init__(
        self, master, mapper, archive, network, external, display, setLive, pqueue
    ):
        ttk.Frame.__init__(self, master)
        pt.PrintClient.__init__(self, pqueue)
        ...
        self.liveButton = ttk.Radiobutton(
            self.viewFrame, variable=self.viewVar, value=self.DM_LIVE, text="Live"
        )
        self.liveButton.pack(side=tk.TOP, padx=5, fill=tk.X, expand=True)
        self.builderButton = ttk.Radiobutton(
            self.viewFrame, variable=self.viewVar, value=self.DM_BUILDER, text="Preview"
        )
        self.builderButton.pack(side=tk.TOP, padx=5, fill=tk.X, expand=True)
        ...
        self.selectAllButton = ttk.Button(
            self.selectFrame, text="Select All", command=self.selectAll
        )
        self.selectAllButton.pack(side=tk.TOP, padx=5, fill=tk.X, expand=True)
        ...
        self.deselectAllButton.pack(side=tk.TOP, padx=5, fill=tk.X, expand=True)
        ...
        # Functional ...........................................................
        self.functional = FunctionControlWidget(
            self.notebook, display, self._onRecordStart, self._onRecordStop, pqueue
        )
        self.notebook.add(self.functional, text="Scripting", state=tk.NORMAL)

        # External .............................................................
        self.external = ex.ExternalControlWidget(
            self.notebook, self.archive, self.externalBackEnd, pqueue
        )  # FIXME pass backend
        self.notebook.add(self.external, text="External", state=tk.NORMAL)
        ...
        self.fileLabel = ttk.Label(
            self.fileFrame, text="File: ", style="Secondary.TLabel"
        )
        self.fileLabel.pack(side=tk.LEFT)
        self.fileField = ttk.Entry(self.fileFrame, width=20, state=tk.DISABLED)
        self.fileField.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.fileButton = ttk.Button(
            self.fileFrame,
            text="...",
            command=self._onFileButton,
            style="Secondary.TButton",
        )
        ...
        self.recordControlFrame.pack(side=tk.TOP, fill=tk.X, expand=True, **gus.padc)
        self.recordStartButton = ttk.Button(
            self.recordControlFrame, text="Start", command=self._onRecordStart
        )
        self.recordStartButton.pack(side=tk.LEFT)
        self.recordPauseButton = ttk.Button(
            self.recordControlFrame,
            text="Pause",
            state=tk.DISABLED,
        ...
        self.recordIndexButton = ttk.Checkbutton(
            self.recordControlFrame, text="Auto Index", variable=self.recordIndexVar
        )
        self.recordIndexButton.pack(side=tk.LEFT, **gus.padc)
        self.activeWidgets.append(self.recordIndexButton)

        self.nextIndexLabel = ttk.Label(
            self.recordControlFrame, text="  Next: ", style="Secondary.TLabel"
        )
        self.nextIndexLabel.pack(side=tk.LEFT)
        validateN = self.register(gus._validateN)
        self.nextIndexEntry = ttk.Entry(
            self.recordControlFrame,
            width=6,
            validate="key",
    ...
    def _getDefaultFile(self):
        ...
        """
        return "FC_recording_on_{}.csv".format(
    ...
    def _onFileButton(self, *_):
        ...
        if fetched is not None and len(fetched) > 0 and fetched != self.fullname:
    ...
    def _getFile(self):
        ...
        """
        return fdg.asksaveasfilename(
            initialdir=self.logDirectory,
            title="Set Log File",
            initialfile=(
                self.filename if self.filename is not None else self._getDefaultFile()
            ),
    ...
    def _onRecordStart(self, event=None):
        ...
        self.fileField.config(state=tk.DISABLED)
        self.recordStartButton.config(
            text="Stop", command=self._onRecordStop
    ...
    def _onRecordStop(self, event=None):
        ...
        self.recordStartButton.config(
            text="Start", command=self._onRecordStart
        )  # Record start functionality
        self._setActiveWidgets(tk.NORMAL)
...
class DisplayMaster(ttk.Frame, pt.PrintClient):
    ...
    def __init__(self, master, pqueue):
        ...
        self.menuFrame.grid(row=self.MENU_ROW, column=self.MENU_COLUMN, sticky="EW")
        ...
        self.deltaDisplay = ttk.Label(
            self.menuFrame,
            style="Secondary.TLabel",
            textvariable=self.deltaVar,
            width=5,
        )
    ...
    def add(self, display, text, **pack_kwargs):
        ...
        button = ttk.Radiobutton(
            self.selectorFrame, variable=self.selected, value=index, text=text
        )
...
class GridWidget(gd.BaseGrid, pt.PrintClient):
    ...
    def __init__(
        ...
        gd.BaseGrid.__init__(
            self, master, self.R, self.C, cursor=self.CURSOR, empty=self.empty_color
        )
        pt.PrintClient.__init__(self, pqueue)
        ...
        self.deepButton = ttk.Checkbutton(
            self.toolBar, text="Deep Select", variable=self.deepVar
        )
        ...
        self.selectButton = ttk.Radiobutton(
            self.toolBar,
            variable=self.selectMode,
            value=self.SM_SELECT,
            text="Rectangle",
        )
        self.selectButton.pack(side=tk.LEFT)
        self.drawButton = ttk.Radiobutton(
            self.toolBar, text="Trace", variable=self.selectMode, value=self.SM_DRAW
        )
        ...
        self.holdButton = ttk.Checkbutton(
            self.toolBar, text="Hold Selection", variable=self.holdVar
        )
        ...
        self.colorBar = ColorBarWidget(
            self,
            colors=cms.COLORMAP_GALCIT,
            high=self.maxRPM,
            unit="RPM",
            pqueue=pqueue,
        )
        self.colorBar.grid(row=self.GRID_ROW, column=self.GRID_COLUMN + 1, sticky="NS")
    ...
    def map(self, func, t=0, t_step=0):
        ...
        try:
            ...
            if (
                hasattr(self, "F_buffer")
                and self.F_buffer
                and len(self.F_buffer) >= 2 * self.size_k
            ):
            ...
            for k in self.range_k:
                ...
                if self.selected_count == 0 or (g >= 0 and self.selected_g[g]):
                    self.control_buffer[k] = func(
                        r,
                        c,
                        l,
                        s,
                        f,
                        self.F_buffer[self.size_k + k],
                        self.F_buffer[k],
                        self.R,
                        self.C,
                        self.L,
    ...
    def update_g(self, g, value):
        ...
        if g < 0 or g >= len(self.values_g):
            self.printw(
                f"Grid index {g} out of bounds (array size: {len(self.values_g)}). Ignoring."
            )
            return
        ...
        if value >= 0:
            fill = self.colors[
                min(self.maxColor, int(((value * self.maxColor) / self.maxValue)))
            ]
        elif value == std.RIP:
            fill = self.off_color
    ...
    def select_g(self, g):
        ...
        if g < 0 or g >= len(self.selected_g):
            self.printw(
                f"Grid index {g} out of bounds for selection (array size: {len(self.selected_g)}). Ignoring."
            )
            return
        ...
        if self.layer_g(g) == self.layer:
            self.outlinei(self.gridi_g(g), self.OUTLINE_SELECTED, self.WIDTH_SELECTED)

    def deselect_g(self, g):
        # Add bounds checking to prevent IndexError
        if g < 0 or g >= len(self.selected_g):
            self.printw(
                f"Grid index {g} out of bounds for deselection (array size: {len(self.selected_g)}). Ignoring."
            )
            return
        ...
        if self.layer_g(g) == self.layer:
            self.outlinei(self.gridi_g(g), self.OUTLINE_NORMAL, self.WIDTH_NORMAL)
    ...
    def _generalRelease(grid, i, f_i):
        if (
            grid.drag_start != grid.drag_end
            and grid.drag_start != None
            and grid.drag_end != None
        ):
...
class ColorBarWidget(ttk.Frame):
    ...
    def __init__(self, master, colors, pqueue, high=100, unit="[?]"):
        ...
        # Widgets ..............................................................
        self.highLabel = ttk.Label(
            self, text="{} {}".format(high, unit), style="Secondary.TLabel"
        )
        self.highLabel.grid(row=0, sticky="EW")

        self.canvas = tk.Canvas(
            self, bg=self.colors[-1], width=self.highLabel.winfo_width()
        )
        self.canvas.grid(row=1, sticky="NEWS")

        self.lowLabel = ttk.Label(
            self, text="{} {}".format(self.low, unit), style="Secondary.TLabel"
        )
...
class LiveTable(pt.PrintClient, ttk.Frame):
    ...
    def __init__(self, master, archive, mapper, send_method, network, pqueue):
        ...
        self.topBar.grid(row=self.MENU_ROW, column=self.MENU_COLUMN, sticky="EW")

        self.offset = 0
        self.showMenuVar = tk.StringVar()
        self.showMenuVar.trace("w", self._showMenuCallback)
        self.showMenuVar.set("RPM")
        self.showMenu = ttk.OptionMenu(
            self.topBar, self.showMenuVar, self.showMenuVar.get(), "RPM", "DC"
        )
        ...
        self.playPauseButton = ib.create_pause_button(
            self.topBar, command=self._playPause
        )
        ...
        self.sentinelTerciaryLabel = ttk.Label(
            self.sentinelFrame, style="Secondary.TLabel", text=" RPM   "
        )
        ...
        self.tableFrame.grid(
            row=self.TABLE_ROW, column=self.TABLE_COLUMN, sticky="NEWS"
        )
        self.table = ttk.Treeview(self.tableFrame, height=32)
        ...
        self.style.configure(
            "Treeview.Heading", font=gus.typography["label_small"]["font"]
        )
        ...
        self.boldFontSettings = (
            gus.typography["code"]["font"][0],
            gus.typography["code"]["font"][1],
            "bold",
        )
        ...
        for column in self.columns[: self.specialColumns]:
            self.table.column(
                column, anchor="center", stretch=False, width=self.specwidth
            )
            self.table.heading(column, text=column)

        for column in self.columns[self.specialColumns : -1]:
            self.table.column(
                column, width=self.rpmwidth, anchor="center", stretch=False
            )
            self.table.heading(column, text=column)

        self.table.column(
            self.columns[-1], width=self.rpmwidth, anchor="center", stretch=True
        )
        ...
        self.table.tag_configure("stripe_even", background=SURFACE_1)
        self.table.tag_configure(
            "stripe_odd",
        ...
        self.hscrollbar.grid(
            row=self.HSCROLL_ROW, column=self.HSCROLL_COLUMN, sticky="EW"
        )
        ...
        self.vscrollbar.grid(
            row=self.VSCROLL_ROW, column=self.VSCROLL_COLUMN, sticky="NS"
        )
    ...
    def deactivatei(self, i):
        ...
        """
        self.table.item(self.slaves[i], values=(i + 1,), tag="D")
    ...
    def _executeSentinel(self, targetSlave, targetFan, RPM):
        ...
        if action == "Highlight row(s)":
            ...
            self._printe(
                "WARNING: Module {}, Fan {} at {} RPM".format(
            ...
            self._printe(
                "WARNING: Shutdown triggered by Module {}, Fan {} "
                "({} RPM)".format(targetSlave + 1, targetFan, RPM)
            )

        # Print (and avoid printing the same matrix twice):
        if self.sentinelPrintVar.get() and self.lastPrintedMatrix < self.matrixCount:
    ...
    def _assembleSentinel(self):
        ...
        if check == "Above":
            ...
            return lambda rpm: rpm > value * 1.1 or rpm < value * 0.9

        elif check == "Within 10% of":
            return lambda rpm: rpm < value * 1.1 and rpm > value * 0.9
    ...
    def _printMatrix(self, event=None, sentinelValues=None):
        ...
        self.printThread = threading.Thread(
            name="FCMkII_LT_Printer", target=self._printRoutine, args=(sentinelValues,)
        )
    ...
    def _printRoutine(self, sentinel=None):
        ...
        try:
            ...
            with open(fileName, "w") as f:
                ...
                f.write(
                    "Fan Club MkII data launched on {}  using "
                ...
                if sentinel is not None:
                    f.write(
                        "NOTE: This data log was activated by a watchdog "
        ...
        except (IOError, ValueError, TypeError, MemoryError) as e:
            self._printM("ERROR When printing matrix:{}".format(str(e)), "E")
            self.donePrinting = True
        except Exception as e:
            self._printM("UNEXPECTED ERROR When printing matrix:{}".format(str(e)), "E")
            self.donePrinting = True
        ...
        if self.playPauseFlag:
            ...
            if N > self.numSlaves:
                for index in range(self.numSlaves, N):
                    ...
                    tags = ("N", stripe_tag)
                    slave_id = table_insert(
                        "", "end", values=(index + 1,) + self.zeroes, tags=tags
                    )
                    # Bug fix: slaves is a dict {index: slave_id}, not a list
...
class DataLogger(pt.PrintClient):
    ...
    def start(
        ...
        try:
            ...
            self.process = mp.Process(
                ...
                args=(
                    filename,
                    self.archive[ac.version],
                    self.slaves,
                    self.archive[ac.name],
                    self.archive[ac.maxFans],
    ...
    def _routine(
        ...
        with open(filename, "w") as f:
            ...
            f.write(
                "Fan Club MkIV ({}) data log started on {}  using "
                'profile "{}"\n'.format(
                    version,
            ...
            # (Header) Dimensions:
            f.write(
                "Dimensions (rows, columns, layers): {}x{}x{}\n".format(*dimensions)
            )
            ...
            # Header (3/4)
            f.write(
                "Column headers are of the form s[MODULE#][type][FAN#]"
