...
class FilterConfigWidget(ttk.Frame, pt.PrintClient):
    ...
    def __init__(self, master, archive=None, pqueue=None, monitoring_widget=None):
        ...
        if self.monitoring_widget and hasattr(self.monitoring_widget, "tach_config"):
            self.enabled_fans = self.monitoring_widget.tach_config.enabled_fans[
                :8
            ]  # 最多8个风机
        else:
    ...
    def create_filter_widgets(self):
        ...
        self.control_frame = ttk.LabelFrame(
            self.main_frame, text="滤波器配置", padding=10
        )
    ...
    def create_filter_type_widgets(self):
        """创建滤波器类型选择组件"""
        self.type_frame = ttk.LabelFrame(
            self.control_frame, text="滤波器类型", padding=5
        )

        # 滤波器类型选择
        ttk.Label(self.type_frame, text="类型:").grid(
        ...
        # 滤波方法选择
        ttk.Label(self.type_frame, text="方法:").grid(
    ...
    def create_basic_params_widgets(self):
        ...
        self.basic_frame = ttk.LabelFrame(
            self.control_frame, text="基本参数", padding=5
        )

        # 采样率
        ttk.Label(self.basic_frame, text="采样率 (Hz):").grid(
            row=0, column=0, sticky="w", padx=5, pady=2
        )
        self.sampling_rate_var = tk.StringVar(value="1000")
        self.sampling_rate_entry = ttk.Entry(
            self.basic_frame, textvariable=self.sampling_rate_var, width=15
        )
        self.sampling_rate_entry.grid(row=0, column=1, padx=5, pady=2)

        # 截止频率
        ttk.Label(self.basic_frame, text="截止频率 (Hz):").grid(
            row=1, column=0, sticky="w", padx=5, pady=2
        )
        self.cutoff_freq_var = tk.StringVar(value="100")
        self.cutoff_freq_entry = ttk.Entry(
            self.basic_frame, textvariable=self.cutoff_freq_var, width=15
        )
        self.cutoff_freq_entry.grid(row=1, column=1, padx=5, pady=2)

        # 滤波器阶数
        ttk.Label(self.basic_frame, text="阶数:").grid(
            row=2, column=0, sticky="w", padx=5, pady=2
        )
        self.order_var = tk.StringVar(value="4")
        self.order_entry = ttk.Entry(
            self.basic_frame, textvariable=self.order_var, width=15
        )
    ...
    def create_advanced_params_widgets(self):
        ...
        self.advanced_frame = ttk.LabelFrame(
            self.control_frame, text="高级参数", padding=5
        )
        ...
        # 卡尔曼滤波参数
        ttk.Label(self.kalman_frame, text="过程噪声:").grid(
            row=0, column=0, sticky="w", padx=5, pady=2
        )
        self.process_noise_var = tk.StringVar(value="0.01")
        self.process_noise_entry = ttk.Entry(
            self.kalman_frame, textvariable=self.process_noise_var, width=15
        )
        self.process_noise_entry.grid(row=0, column=1, padx=5, pady=2)

        ttk.Label(self.kalman_frame, text="观测噪声:").grid(
            row=1, column=0, sticky="w", padx=5, pady=2
        )
        self.observation_noise_var = tk.StringVar(value="0.1")
        self.observation_noise_entry = ttk.Entry(
            self.kalman_frame, textvariable=self.observation_noise_var, width=15
        )
        self.observation_noise_entry.grid(row=1, column=1, padx=5, pady=2)

        # Huber鲁棒滤波参数
        ttk.Label(self.huber_frame, text="阈值:").grid(
            row=0, column=0, sticky="w", padx=5, pady=2
        )
        self.huber_threshold_var = tk.StringVar(value="1.345")
        self.huber_threshold_entry = ttk.Entry(
            self.huber_frame, textvariable=self.huber_threshold_var, width=15
        )
        self.huber_threshold_entry.grid(row=0, column=1, padx=5, pady=2)

        ttk.Label(self.huber_frame, text="窗口大小:").grid(
            row=1, column=0, sticky="w", padx=5, pady=2
        )
        self.huber_window_var = tk.StringVar(value="10")
        self.huber_window_entry = ttk.Entry(
            self.huber_frame, textvariable=self.huber_window_var, width=15
        )
        self.huber_window_entry.grid(row=1, column=1, padx=5, pady=2)

        # α-β滤波参数
        ttk.Label(self.alpha_beta_frame, text="α因子:").grid(
            row=0, column=0, sticky="w", padx=5, pady=2
        )
        self.alpha_var = tk.StringVar(value="0.8")
        self.alpha_entry = ttk.Entry(
            self.alpha_beta_frame, textvariable=self.alpha_var, width=15
        )
        self.alpha_entry.grid(row=0, column=1, padx=5, pady=2)

        ttk.Label(self.alpha_beta_frame, text="β因子:").grid(
            row=1, column=0, sticky="w", padx=5, pady=2
        )
        self.beta_var = tk.StringVar(value="0.2")
        self.beta_entry = ttk.Entry(
            self.alpha_beta_frame, textvariable=self.beta_var, width=15
        )
    ...
    def create_control_buttons(self):
        ...
        self.apply_button = ttk.Button(
            self.button_frame, text="应用滤波器", command=self.apply_filter
        )
        self.apply_button.pack(side="left", padx=5, pady=5)

        # 重置按钮
        self.reset_button = ttk.Button(
            self.button_frame, text="重置", command=self.reset_config
        )
        ...
        self.load_button = ttk.Button(
            self.button_frame, text="加载配置", command=self.load_config
        )
        self.load_button.pack(side="left", padx=5, pady=5)

    def create_frequency_response_plot(self):
        """创建频率响应图"""
        self.freq_plot_frame = ttk.LabelFrame(
            self.display_frame, text="频率响应", padding=5
        )
    ...
    def create_time_domain_plot(self):
        """创建时域预览图（显示实时tach数据）"""
        self.time_plot_frame = ttk.LabelFrame(
            self.display_frame, text="实时Tach信号监控", padding=5
        )
    ...
    def create_tach_control_panel(self):
        """创建tach数据控制面板"""
        self.tach_control_frame = ttk.LabelFrame(
            self.control_frame, text="Tach数据控制", padding=5
        )
        ...
        self.fan_entry = ttk.Entry(
            fan_frame, textvariable=self.fan_selection_var, width=15
        )
        ...
        self.start_button = ttk.Button(
            button_frame, text="开始监控", command=self.toggle_monitoring
        )
        self.start_button.pack(side="left", padx=2)

        self.clear_button = ttk.Button(
            button_frame, text="清除数据", command=self.clear_tach_data
        )
    ...
    def bind_events(self):
        ...
        for var in [
            ...
            self.filter_method_var,
            self.sampling_rate_var,
            self.cutoff_freq_var,
            self.order_var,
        ]:
    ...
    def is_tach_monitoring_active(self):
        """检查tach监控是否激活"""
        if self.monitoring_widget and hasattr(
            self.monitoring_widget, "tach_monitoring_active"
        ):
    ...
    def schedule_filter_update(self):
        ...
        try:
            ...
            if (
                ...
                or not hasattr(self.master, "winfo_exists")
                or not self.master.winfo_exists()
            ):
                return

            # Check if we have the necessary methods
            if not hasattr(self, "is_tach_monitoring_active") or not hasattr(
            ...
            if self.is_tach_monitoring_active():
                ...
                try:
                    ...
                    if (
                        hasattr(self, "winfo_exists")
                        and self.winfo_exists()
                        and hasattr(self, "is_tach_monitoring_active")
                        and self.is_tach_monitoring_active()
                        and hasattr(self, "after")
                    ):
            ...
            for fan_id in self.enabled_fans:
                if fan_id in tach_data and tach_data[fan_id].get("raw_signals"):
                    ...
                    if (
                        not raw_signals
                        or not timestamps
                        or len(raw_signals) != len(timestamps)
                    ):
                    ...
                    # 应用滤波器到原始信号
                    filtered_rpm = self._apply_filter_to_signal(
                        raw_signals, filter_type, config
                    )
                    ...
                    if filtered_rpm:
                        ...
                        self.filtered_data[fan_id]["timestamps"] = deque(
                            timestamps, maxlen=1000
                        )
                        self.filtered_data[fan_id]["filtered_rpm"] = deque(
                            filtered_rpm, maxlen=1000
                        )
                ...
                for i, fan_id in enumerate(self.enabled_fans[:4]):  # 最多显示4个风机
                    if fan_id in tach_data and tach_data[fan_id]["timestamps"]:
                        ...
                        if timestamps:
                            ...
                            # 绘制滤波后的数据
                            if (
                                fan_id in self.filtered_data
                                and self.filtered_data[fan_id]["filtered_rpm"]
                            ):
                                filtered_rpm = list(
                                    self.filtered_data[fan_id]["filtered_rpm"]
                                )
                                self.time_ax.plot(
                                    rel_times,
                                    filtered_rpm,
    ...
    def on_fan_selection_change(self, event=None):
        ...
        try:
            ...
            fan_ids = [
                int(x.strip()) for x in fan_str.split(",") if x.strip().isdigit()
            ]
            ...
            if self.monitoring_widget and hasattr(
                self.monitoring_widget, "tach_config"
            ):
        ...
        try:
            ...
            if filter_method in ["BUTTERWORTH", "IIR"]:
                ...
                if filter_type == "LOWPASS":
                    ...
                    b = signal.firwin(
                        fir_length, normalized_fc, window="hamming", pass_zero=False
                    )
                else:
                    b = signal.firwin(fir_length, normalized_fc, window="hamming")
                a = [1.0]  # FIR滤波器的分母系数
    ...
    def safe_update_plots(self):
        ...
        try:
            ...
            if (
                ...
                and not self.master.winfo_exists()
            ):
                return
    ...
    def update_plots(self, *args):
        ...
        try:
            ...
            if b is not None and a is not None:
                # 计算频率响应
                w, h = signal.freqz(
                    b, a, worN=1024, fs=config["sampling_rate"]
                )  # 减少计算量
                ...
                # 添加截止频率线
                self.freq_ax.axvline(
                    config["cutoff_frequency"],
                    color="r",
                    linestyle="--",
                    alpha=0.7,
                    label=f"fc = {config['cutoff_frequency']:.1f} Hz",
                )

                # 添加-3dB线
                self.freq_ax.axhline(
                    -3, color="g", linestyle=":", alpha=0.7, label="-3dB"
                )

                self.freq_ax.set_xlabel("频率 (Hz)")
                self.freq_ax.set_ylabel("幅度 (dB)")
                self.freq_ax.set_title(
                    f'{config["filter_type"]} {config["filter_method"]} 滤波器频率响应'
                )
                ...
                self.status_var.set(
                    f"频率响应已更新 - {config['filter_type']} {config['filter_method']}"
                )
            else:
                # 如果滤波器设计失败，显示空图表
                self.freq_ax.clear()
    ...
    def apply_filter(self):
        ...
        try:
            ...
            self.status_var.set(
                f"滤波器已应用 - {config['filter_type']} {config['filter_method']}"
            )
        ...
        try:
            config = {
                ...
                "observation_noise": max(
                    0.001, float(self.observation_noise_var.get())
                ),
            ...
            if config["cutoff_frequency"] >= config["sampling_rate"] / 2:
                ...
                self.printw(
                    f"截止频率已调整为 {config['cutoff_frequency']:.1f} Hz (奈奎斯特频率限制)"
                )
