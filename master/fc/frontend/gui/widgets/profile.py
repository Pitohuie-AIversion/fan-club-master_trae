...
class ProfileDisplay(ttk.Frame, pt.PrintClient):
    ...
    def __init__(self, master, archive, callback, pqueue):
        ...
        self.topLabel = ttk.Label(self.topBar, text="Profile:   ", justify=tk.LEFT)
        self.topLabel.pack(side=tk.LEFT)

        self.defaultButton = ttk.Button(
            self.topBar, text="Default", command=self._default
        )
        ...
        self.applyButton = ttk.Button(
            self.topBar, text="Apply", command=self.applyCallback
        )
        ...
        self.builtinLabel = ttk.Label(self.builtinFrame, text="Built-in: ")
        ...
        self.builtinMenu = ttk.Combobox(
            self.builtinFrame,
            textvariable=self.builtinMenuVar,
            values=builtinkeys,
            state="readonly",
        )
        ...
        self.display.column("#0", width=self.font.measure("    "), stretch=False)
        ...
        for event in ("<ButtonRelease-1>", "<KeyRelease-Up>", "<KeyRelease-Down>"):
        ...
        self.editor = PythonEditor(
            ...
            edit_callback=self._on_edit,
            remove_callback=self._on_remove,
            printx=self._nothing,
            printr=self._nothing,
        )
    ...
    def build(self):
        ...
        """
        self.clear()
        self.root = self._addModule(self.archive[ac.name], self.archive.profile(), 0)
    ...
    def _addModule(self, name, module, precedence, parent=""):
        ...
        """
        iid = self.display.insert(parent, precedence, values=(name, ""), tag=TAG_SUB)
        for child in module:
            meta = self.archive.meta[child]
            if meta[ac.TYPE] is ac.TYPE_LIST:
                self._addList(meta[ac.NAME], module[child], meta[ac.PRECEDENCE], iid)
            elif meta[ac.TYPE] is ac.TYPE_SUB:
                self._addModule(meta[ac.NAME], module[child], meta[ac.PRECEDENCE], iid)
            elif meta[ac.TYPE] is ac.TYPE_MAP:
                self._addMap(meta[ac.NAME], module[child], meta[ac.PRECEDENCE], iid)
            else:
                self._addPrimitive(
                    meta[ac.NAME], module[child], meta[ac.PRECEDENCE], iid
                )
        self.map[iid] = (name, module)
        return iid

    def _addPrimitive(self, name, value, precedence, parent=""):
        iid = self.display.insert(parent, precedence, values=(name, repr(value)))
    ...
    def _addList(self, name, iterable, precedence, parent=""):
        iid = self.display.insert(parent, precedence, values=(name, ""), tag=TAG_LIST)

        indexer = self.archive.defaults[ac.INVERSE[name]][ac.INDEXER]
        for element in iterable:
            # Add list elements as modules or primitives based on type
            element_iid = self._addModule(
                indexer(element), element, indexer(element), iid
            )
    ...
    def _check_editability(self, event=None):
        ...
        parent_key = (
            ac.INVERSE[self.display.item(parent_iid)["values"][0]]
            if parent_iid != self.root
            else None
        )
        T_parent = self.archive.meta[parent_key][ac.TYPE] if parent_key else None
    ...
    def _schedule_update_check(self):
        ...
        try:
            ...
            if not hasattr(self, "_auto_update_enabled") or not hasattr(
    ...
    def _check_archive_updates(self):
        ...
        try:
            ...
            if (
                self._last_archive_hash is not None
                and current_hash != self._last_archive_hash
            ):
                if hasattr(self, "printx"):
                    self.printx(
                        "[AUTO-UPDATE] Archive modified externally, refreshing display..."
                    )
    ...
    def _loadBuiltin(self, name):
        ...
        try:
            ...
            if not isinstance(profile_data, dict):
                self.printx(
                    f"Error: Invalid profile data for '{name}' - expected dictionary"
                )
                return False
            ...
            if missing_fields:
                self.printx(
                    f"Warning: Profile '{name}' missing fields: {missing_fields}"
                )
...
class PythonEditor(ttk.Frame):
    ...
    def __init__(
        self, master, add_callback, edit_callback, remove_callback, printr, printx
    ):
        ...
        self.topLabel = ttk.Label(
            self, text="Value editor (as Python 3.7 expression):", anchor=tk.W
        )
        ...
        self.input = tk.Text(self, width=30, height=2, padx=10, pady=0, **conf)
        ...
        self.input_scrollbar.grid(row=row, column=2, rowspan=1, sticky="NS")
        ...
        self.output_scrollbar.grid(row=row, column=2, rowspan=1, sticky="NS")
    ...
    def _eval_error(self, e):
        ...
        if isinstance(e, SyntaxError):
            ...
            user_message = (
                f"Name Error: Undefined variable or function\n{error_message}"
            )
            ...
            user_message = (
                f"Attribute Error: Missing attribute or method\n{error_message}"
            )
