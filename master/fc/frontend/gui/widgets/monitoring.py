...
class TachSignalConfig:
    ...
    simulation_diagnostics: bool = (
        False  # Enable/disable diagnostics for simulated data
    )
...
class MonitoringWidget(ttk.Frame, pt.PrintClient):
    ...
    def _build_control_bar(self):
        ...
        self.status_label = ttk.Label(
            status_frame, text="Stopped", foreground=ERROR_MAIN
        )
        self.status_label.grid(row=0, column=1, padx=5)

        # Clear data button
        self.clear_button = ttk.Button(
            control_frame, text="Clear Data", command=self._clear_data
        )
    ...
    def _build_signal_tab(self):
        ...
        ttk.Label(signal_control, text="Sample Rate (Hz):").grid(
            row=0, column=2, padx=5
        )
        self.sample_rate_var = tk.StringVar(value="1000")
        sample_rate_entry = ttk.Entry(
            signal_control, textvariable=self.sample_rate_var, width=10
        )
    ...
    def _build_signal_plot(self, parent):
        ...
        plot_frame = ttk.LabelFrame(
            parent, text="Real-time Signal Waveform", padding=10
        )
        ...
        for i in range(3):  # 3 channels
            (line,) = self.signal_ax.plot(
                [], [], color=CHANNEL_COLORS[i], label=f"Channel {i+1}", linewidth=2
            )
    ...
    def _build_signal_text(self, parent):
        ...
        scrollbar = ttk.Scrollbar(
            text_frame, orient="vertical", command=self.signal_text.yview
        )
    ...
    def _build_tach_control_panel(self, parent):
        ...
        tach_check = ttk.Checkbutton(
            control_frame,
            text="Enable Tach Monitoring",
        ...
        filter_window_entry = ttk.Entry(
            control_frame, textvariable=self.filter_window_var, width=8
        )
        ...
        rpm_threshold_entry = ttk.Entry(
            control_frame, textvariable=self.rpm_threshold_var, width=8
        )
    ...
    def _build_tach_status_panel(self, parent):
        ...
        for i in range(6):  # Display status of first 6 fans
            ...
            self.tach_status_labels[i] = {"rpm": rpm_label, "status": status_label}

    def _build_tach_plot(self, parent):
        """Build Tach signal plot display"""
        plot_frame = ttk.LabelFrame(
            parent, text="Tach Signal Real-time Waveform", padding=10
        )
        ...
        self.tach_ax_raw.set_title(
            "Raw Tach Signal Monitoring", color=TEXT_PRIMARY, fontsize=12
        )
        ...
        self.tach_ax1.set_title(
            "Fan Speed Real-time Monitoring", color=TEXT_PRIMARY, fontsize=12
        )
        ...
        self.tach_ax2.set_title(
            "Current Speed Distribution", color=TEXT_PRIMARY, fontsize=12
        )
    ...
    def _build_tach_text(self, parent):
        ...
        # Create text display area
        self.tach_text = tk.Text(
            text_frame, height=20, bg=SURFACE_2, fg=TEXT_PRIMARY, font=("Consolas", 10)
        )
        tach_scrollbar = ttk.Scrollbar(
            text_frame, orient="vertical", command=self.tach_text.yview
        )
    ...
    def _build_performance_plot(self, parent):
        ...
        (self.cpu_line,) = self.perf_ax.plot(
            [], [], color=SUCCESS_MAIN, label="CPU", linewidth=2
        )
        (self.mem_line,) = self.perf_ax.plot(
            [], [], color=WARNING_MAIN, label="Memory", linewidth=2
        )
    ...
    def _build_performance_text(self, parent):
        ...
        perf_scrollbar = ttk.Scrollbar(
            text_frame, orient="vertical", command=self.perf_text.yview
        )
    ...
    def _build_statistics_tab(self):
        ...
        stats_text_frame = ttk.LabelFrame(
            stats_frame, text="Statistics Information", padding=10
        )
        ...
        stats_scrollbar = ttk.Scrollbar(
            stats_text_frame, orient="vertical", command=self.stats_text.yview
        )
    ...
    def _apply_tach_filter(self, fan_id: int, rpm_value: float) -> float:
        ...
        if (
            not self.tach_config.filter_enabled
            or self.tach_config.filter_type == "none"
        ):
            return rpm_value
        ...
        if self.tach_config.filter_type == "moving_average":
            ...
            filtered_value = (
                alpha * rpm_value + (1 - alpha) * filter_state["last_filtered"]
            )
    ...
    def _diagnose_tach_signal(
        self, fan_id: int, reading: TachReading, filtered_rpm: float
    ):
        ...
        try:
            ...
            if not hasattr(self, "fc_communicator") or self.fc_communicator is None:
                ...
                if reading.timeout and self.tach_config.simulation_diagnostics:
                    ...
                    if (
                        fan_id not in self._last_timeout_log
                        or current_time - self._last_timeout_log[fan_id] > 30
                    ):
            ...
            if reading.timeout:
                if (
                    fan_id not in self._last_diagnostic_log
                    or current_time - self._last_diagnostic_log[fan_id] > 10
                ):
            ...
            if not reading.timeout:
                ...
                if filtered_rpm < self.tach_config.rpm_threshold:
                    if (
                        fan_id not in self._last_diagnostic_log
                        or current_time - self._last_diagnostic_log[fan_id] > 15
                    ):
                        self.printw(
                            f"Fan {fan_id+1} RPM too low: {filtered_rpm:.0f} < {self.tach_config.rpm_threshold}"
                        )
                        self._last_diagnostic_log[fan_id] = current_time
                ...
                if filtered_rpm > 5000:  # Assume 5000 RPM as abnormally high value
                    if (
                        ...
                        self.printw(
                            f"Fan {fan_id+1} RPM abnormally high: {filtered_rpm:.0f}"
                        )
                        self._last_diagnostic_log[fan_id] = current_time

                # Check if RPM fluctuation is too large
                if (
                    fan_id in self.tach_data
                    and len(self.tach_data[fan_id]["filtered_rpm"]) > 5
                ):
                    ...
                    if rpm_std > 200:  # RPM standard deviation too large
                        if (
                            fan_id not in self._last_diagnostic_log
                            or current_time - self._last_diagnostic_log[fan_id] > 20
                        ):
                            self.printw(
                                f"Fan {fan_id+1} RPM fluctuation too large: std={rpm_std:.1f}"
                            )
                            self._last_diagnostic_log[fan_id] = current_time
            ...
            if reading.duty_cycle < 0 or reading.duty_cycle > 1:
                if (
                    ...
                    self.printw(
                        f"Fan {fan_id+1} duty cycle abnormal: {reading.duty_cycle:.2f}"
                    )
                    self._last_diagnostic_log[fan_id] = current_time
            ...
            if hasattr(self.fc_communicator, "get_rpm_data"):
                ...
                if rpm_data:
                    for fan_id, rpm_value in enumerate(rpm_data):
                        if fan_id in self.tach_config.enabled_fans and fan_id < len(
                            rpm_data
                        ):
    ...
    def _generate_simulated_tach_data(self, timestamp):
        ...
        for fan_id in range(21):  # Simulate all 21 fans (maximum supported)
            if fan_id in self.tach_config.enabled_fans:
                ...
                raw_signal = 3.3 * (
                    0.5 + 0.5 * math.sin(2 * math.pi * signal_freq * timestamp)
                )
    ...
    def _update_system_info(self):
        ...
        if self.system_stats["timestamps"]:
            ...
            cpu_usage = (
                self.system_stats["cpu_usage"][-1]
                if self.system_stats["cpu_usage"]
                else 0
            )
            mem_usage = (
                self.system_stats["memory_usage"][-1]
                if self.system_stats["memory_usage"]
                else 0
            )
            packet_count = (
                self.system_stats["network_packets"][-1]
                if self.system_stats["network_packets"]
                else 0
            )
            ...
            self.system_info_labels["Connected Devices"].configure(
                text="3"
            )  # Simulated value
            ...
            if len(self.system_stats["timestamps"]) > 1:
                time_diff = (
                    self.system_stats["timestamps"][-1]
                    - self.system_stats["timestamps"][-2]
                )
                if time_diff > 0:
                    packets_per_sec = 10 / time_diff  # Simulated value
                    self.system_info_labels["Packets/sec"].configure(
                        text=f"{packets_per_sec:.1f}"
                    )
    ...
    def _update_status_bar(self):
        ...
        # Data point count
        total_points = sum(
            len(data["timestamps"]) for data in self.signal_data.values()
        )
    ...
    def _update_statistics(self):
        ...
        stats_info += (
            f"Monitoring Duration: {self.system_stats['timestamps'][-1]:.1f}s\n"
            if self.system_stats["timestamps"]
            else "Monitoring Duration: 0s\n"
        )
                ...
                for fan_id in range(min(6, len(self.tach_data))):
                    if fan_id in self.tach_data:
                        ...
                        if data["timestamps"] and data["raw_signals"]:
                            ...
                            self.tach_ax_raw.plot(
                                timestamps,
                                raw_signals,
                                color=color,
                                label=f"Fan{fan_id+1}",
                                linewidth=1,
                                alpha=0.7,
                            )
        ...
        try:
            ...
            for fan_id in sorted(self.tach_data.keys()):
                ...
                if data["timestamps"]:
                    ...
                    filtered_rpm = (
                        data["filtered_rpm"][-1] if data["filtered_rpm"] else 0
                    )
                    ...
                    if data["filtered_rpm"] and data["timeouts"]:
                        ...
                        if is_timeout:
                            labels["status"].configure(
                                text="Timeout", foreground=ERROR_MAIN
                            )
                        elif current_rpm < self.tach_config.rpm_threshold:
                            labels["status"].configure(
                                text="Low Speed", foreground=WARNING_MAIN
                            )
                        else:
                            labels["status"].configure(
                                text="Normal", foreground=SUCCESS_MAIN
                            )
                    else:
                        labels["rpm"].configure(text="0 RPM")
                        labels["status"].configure(
                            text="Offline", foreground=ERROR_MAIN
                        )
