...
class ModernButton(ttk.Frame):
    ...
    def _on_click(self):
        ...
        self.animation_manager.animate_property(
            ...
            "pack_configure",
            {
                "pady": (0, 2) if not self.is_hovered else (1, 1),
                "padx": (2, 0) if not self.is_hovered else (1, 1),
            },
            {"pady": (2, 0), "padx": (0, 2)},
            duration=100,
            easing=ease_in_out_cubic,
...
class ModernCard(ttk.Frame):
    ...
    def _create_shadow_layers(self):
        ...
        for color, offset in shadow_layers:
            ...
            shadow_frame.pack(
                fill=tk.BOTH, expand=True, pady=(0, offset), padx=(offset, 0)
            )
