...
class FCGUI(fe.FCFrontend):
    ...
    def __init__(self, archive, pqueue):
        ...
        base = bas.Base(
            ...
            title,
            self.version,
            self.addFeedbackClient,
            self.addNetworkClient,
            self.addSlaveClient,
            self._onProfileChange,
    ...
    def _setup_ttk_style(self):
        ...
        try:
            ...
            style.configure(
                "TLabelframe",
                background=base_bg,
                padding=(gus.spacing["sm"], gus.spacing["sm"]),
            )
            style.configure(
                "TLabelframe.Label",
                background=base_bg,
            ...
            style.configure(
                "Warning.TLabel", background=base_bg, foreground=gus.WARNING_MAIN
            )
            ...
            style.map(
                ...
                background=[
                    ...
                    [
                        ...
                        "!focus",
                        gus.PRIMARY_200,
                    ],  # Softer selection when not focused
            ...
            stripe_bg = (
                gus.SURFACE_2 if theme_manager.current_theme == "dark" else "#f8f9fa"
            )
            ...
            # Checkbutton
            style.configure(
                "TCheckbutton", background=base_bg, foreground=fg, padding=(6, 4)
            )
            ...
            # Radiobuttons
            style.configure(
                "TRadiobutton", background=base_bg, foreground=fg, padding=(6, 4)
            )
            style.map("TRadiobutton", foreground=[["disabled", fg_muted]])

            # Combobox
            style.configure(
                "TCombobox", fieldbackground=card_bg, background=card_bg, foreground=fg
            )
            style.map(
                "TCombobox",
            ...
            style.configure(
                "TMenubutton", background=card_bg, foreground=fg, padding=(10, 6)
            )
            style.map(
                "TMenubutton",
