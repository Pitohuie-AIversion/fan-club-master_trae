################################################################################
## Project: Fan Club Mark II "Master" ## File: filter_config_gui.py         ##
##----------------------------------------------------------------------------##
...
class FilterConfigGUI:
    ...
    def create_filter_type_widgets(self):
        ...
        # 绑定事件以动态更新可用选项
        self.filter_method_combo.bind(
            "<<ComboboxSelected>>", self.on_filter_method_change
        )
    ...
    def create_advanced_params_widgets(self):
        ...
        # 通带纹波 (Chebyshev I)
        ttk.Label(advanced_frame, text="通带纹波 (dB):").grid(
            row=0, column=0, sticky="w", padx=5
        )
        self.ripple_var = tk.DoubleVar(value=1.0)
        self.ripple_entry = ttk.Entry(
            advanced_frame, textvariable=self.ripple_var, width=10
        )
        self.ripple_entry.grid(row=0, column=1, sticky="ew", padx=5)

        # 阻带衰减 (Chebyshev II, Elliptic)
        ttk.Label(advanced_frame, text="阻带衰减 (dB):").grid(
            row=1, column=0, sticky="w", padx=5
        )
        self.attenuation_var = tk.DoubleVar(value=40.0)
        self.attenuation_entry = ttk.Entry(
            advanced_frame, textvariable=self.attenuation_var, width=10
        )
        self.attenuation_entry.grid(row=1, column=1, sticky="ew", padx=5)

        # 窗函数类型 (FIR)
        ttk.Label(advanced_frame, text="窗函数:").grid(
        ...
        # 卡尔曼滤波参数
        self.kalman_frame = ttk.LabelFrame(
            advanced_frame, text="卡尔曼滤波参数", padding=5
        )
        ...
        self.process_noise_entry = ttk.Entry(
            self.kalman_frame, textvariable=self.process_noise_var, width=10
        )
        ...
        self.measurement_noise_entry = ttk.Entry(
            self.kalman_frame, textvariable=self.measurement_noise_var, width=10
        )
        self.measurement_noise_entry.grid(row=1, column=1, sticky="ew", padx=5)

        ttk.Label(self.kalman_frame, text="初始估计:").grid(
            row=2, column=0, sticky="w", padx=5
        )
        self.initial_estimate_var = tk.DoubleVar(value=0.0)
        self.initial_estimate_entry = ttk.Entry(
            self.kalman_frame, textvariable=self.initial_estimate_var, width=10
        )
        self.initial_estimate_entry.grid(row=2, column=1, sticky="ew", padx=5)

        ttk.Label(self.kalman_frame, text="初始误差:").grid(
            row=3, column=0, sticky="w", padx=5
        )
        self.initial_error_var = tk.DoubleVar(value=1.0)
        self.initial_error_entry = ttk.Entry(
            self.kalman_frame, textvariable=self.initial_error_var, width=10
        )
        self.initial_error_entry.grid(row=3, column=1, sticky="ew", padx=5)

        self.kalman_frame.grid(
            row=3, column=0, columnspan=2, sticky="ew", padx=5, pady=5
        )
        self.kalman_frame.columnconfigure(1, weight=1)

        # Huber鲁棒滤波参数
        self.robust_frame = ttk.LabelFrame(
            advanced_frame, text="Huber鲁棒滤波参数", padding=5
        )

        ttk.Label(self.robust_frame, text="阈值:").grid(
            row=0, column=0, sticky="w", padx=5
        )
        self.huber_threshold_var = tk.DoubleVar(value=1.345)
        self.huber_threshold_entry = ttk.Entry(
            self.robust_frame, textvariable=self.huber_threshold_var, width=10
        )
        self.huber_threshold_entry.grid(row=0, column=1, sticky="ew", padx=5)

        ttk.Label(self.robust_frame, text="窗口大小:").grid(
            row=1, column=0, sticky="w", padx=5
        )
        self.robust_window_size_var = tk.IntVar(value=10)
        self.robust_window_size_entry = ttk.Entry(
            self.robust_frame, textvariable=self.robust_window_size_var, width=10
        )
        self.robust_window_size_entry.grid(row=1, column=1, sticky="ew", padx=5)

        self.robust_frame.grid(
            row=4, column=0, columnspan=2, sticky="ew", padx=5, pady=5
        )
        self.robust_frame.columnconfigure(1, weight=1)

        # α-β滤波参数
        self.alpha_beta_frame = ttk.LabelFrame(
            advanced_frame, text="α-β滤波参数", padding=5
        )
        ...
        self.alpha_entry = ttk.Entry(
            self.alpha_beta_frame, textvariable=self.alpha_var, width=10
        )
        ...
        self.beta_entry = ttk.Entry(
            self.alpha_beta_frame, textvariable=self.beta_var, width=10
        )
        self.beta_entry.grid(row=1, column=1, sticky="ew", padx=5)

        self.alpha_beta_frame.grid(
    ...
    def create_control_buttons(self):
        ...
        self.apply_btn = ttk.Button(
            button_frame, text="应用滤波器", command=self.apply_filter
        )
        self.apply_btn.pack(side="left", padx=5)

        # 重置按钮
        self.reset_btn = ttk.Button(
            button_frame, text="重置参数", command=self.reset_parameters
        )
    ...
    def create_time_domain_plot(self):
        ...
        signal_combo.bind(
            "<<ComboboxSelected>>", lambda e: self.generate_preview_signal()
        )

        ttk.Button(
            control_frame, text="生成新信号", command=self.generate_preview_signal
    ...
    def update_filter_response(self):
        ...
        try:
            ...
            self.freq_ax1.set_ylabel("幅度 (dB)")
            self.freq_ax1.grid(True, alpha=0.3)
            self.freq_ax1.axvline(
                self.config.cutoff_freq, color="r", linestyle="--", alpha=0.7
            )
    ...
    def design_iir_filter(self):
        """设计IIR滤波器"""
        try:
            filter_type = self.config.filter_type.split("_")[
                1
            ]  # 获取butterworth, chebyshev1等

            if filter_type == "butterworth":
                return signal.butter(
                    self.config.order,
                    self.config.cutoff_freq,
                    btype=self.config.filter_mode,
        ...
        try:
            ...
            if self.config.filter_type == FilterType.FIR_WINDOW:
                ...
                b = signal.firwin(
                    ...
                    normalized_cutoff,
                    window=window,
                    pass_zero=self.config.filter_mode,
                )
                return b, [1.0]
    ...
    def generate_preview_signal(self):
        ...
        try:
            ...
            if signal_type == "sine":
                self.preview_signal = np.sin(2 * np.pi * 50 * t) + 0.5 * np.sin(
                    2 * np.pi * 150 * t
                )
    ...
    def update_time_domain_plot(self):
        ...
        try:
            ...
            self.time_ax.plot(
                t_display,
                self.preview_signal[:display_samples],
                label="原始信号",
                alpha=0.7,
            )
            ...
            if self.filtered_signal is not None:
                self.time_ax.plot(
                    t_display,
                    self.filtered_signal[:display_samples],
                    label="滤波后信号",
                    linewidth=2,
                )
        ...
        try:
            ...
            if filename:
                ...
                self.filter_type_var.set(
                    config_dict.get("filter_type", FilterType.LOWPASS)
                )
                self.filter_method_var.set(
                    config_dict.get("filter_method", FilterMethod.IIR)
                )
                ...
                self.measurement_noise_var.set(
                    config_dict.get("measurement_noise", 0.1)
                )
                ...
                self.robust_window_size_var.set(
                    config_dict.get("robust_window_size", 10)
                )
