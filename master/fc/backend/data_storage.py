...
class DataMetadata:
    ...
    def __init__(
        self, data_type: str, channels: List[str], sample_rate: float, duration: float
    ):
...
class DataExporter(ABC):
    ...
    @abstractmethod
    def export(
        self, data: np.ndarray, metadata: DataMetadata, filepath: str, **kwargs
    ) -> bool:
        """导出数据"""
        pass
...
class SQLiteExporter(DataExporter):
    ...
    def export(
        ...
        try:
            ...
            create_sql = (
                f"CREATE TABLE IF NOT EXISTS signal_data ({', '.join(columns)})"
            )
...
class DataStorageManager:
    ...
    def save_data(
        ...
        metadata: DataMetadata,
        filename: str,
        format_type: DataFormat = DataFormat.HDF5,
        **kwargs,
    ) -> str:
        """保存数据"""
        try:
            ...
            filepath = os.path.join(
                self.config.base_path, f"{base_name}.{format_type.value}"
            )
            ...
            if estimated_size > self.config.max_file_size_mb:
                print(
                    f"警告: 文件大小 {estimated_size:.1f}MB 超过限制 {self.config.max_file_size_mb}MB"
                )
    ...
    def load_data(
        self, filepath: str, format_type: DataFormat = None
    ) -> tuple[np.ndarray, DataMetadata]:
        """加载数据"""
        try:
            # 自动检测格式
