...
class SignalAnalyzer:
    ...
    def calculate_snr(
        self, signal_data: np.ndarray, noise_band: Tuple[float, float] = None
    ) -> float:
        """计算信噪比"""
        try:
            # 计算信号功率
            ...
            if noise_band:
                ...
                sos = signal.butter(
                    4, high_freq_cutoff, "highpass", fs=self.sample_rate, output="sos"
                )
    ...
    def calculate_thd(
        self, signal_data: np.ndarray, fundamental_freq: float = None
    ) -> float:
        """计算总谐波失真"""
        try:
            # 计算功率谱
    ...
    def calculate_stability(
        self, signal_data: np.ndarray, window_size: int = 1024
    ) -> Tuple[float, float]:
        """计算频率和幅度稳定性"""
        try:
            # 分段分析
            ...
            for i in range(num_windows):
                ...
                # 计算主频率
                f, psd = signal.welch(
                    segment, self.sample_rate, nperseg=window_size // 4
                )
...
class AnomalyDetector:
    ...
    def detect_saturation(
        self, signal_data: np.ndarray, channel: str
    ) -> List[AnomalyEvent]:
        """检测信号饱和"""
        anomalies = []
        try:
            ...
            # 检测饱和点
            saturation_mask = (
                np.abs(normalized) > self.detection_thresholds["saturation_threshold"]
            )
...
class QualityMonitor:
    ...
    def analyze_quality(
        self, signal_data: np.ndarray, channel: str = "default"
    ) -> QualityMetrics:
        """分析信号质量"""
        try:
            # 基本统计量
            ...
            freq_stability, amp_stability = self.analyzer.calculate_stability(
                signal_data
            )
    ...
    def get_quality_summary(self, time_window: timedelta = None) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "quality_distribution": {
                level.value: sum(
                    1 for m in filtered_metrics if m.quality_level == level
                )
            ...
            "recent_anomalies": len(
                [
                    ...
                    or a.start_time >= datetime.now() - time_window
                ]
            ),
        }
