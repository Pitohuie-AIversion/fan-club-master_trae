    ...
    def configure_channel(self, channel_config: ChannelConfig) -> bool:
        ...
        try:
            ...
            if channel_config.range_min >= channel_config.range_max:
                pt.print_error(
                    f"Invalid range: min={channel_config.range_min}, max={channel_config.range_max}"
                )
                return False
            ...
            if channel_config.enabled:
                ...
                pt.print_info(
                    f"Channel {channel_id} configured: gain={channel_config.gain}, range=[{channel_config.range_min}, {channel_config.range_max}], coupling={channel_config.coupling}"
                )
            else:
                # 禁用通道
                # 例如: device_api.disable_channel(self.device_handle, channel_id)
            ...
            pt.print_error(
                f"Failed to configure channel {channel_config.channel_id}: {str(e)}"
            )
            return False
...
class SignalAcquisitionEngine(pt.PrintClient):
    ...
    def __init__(self, pqueue, hardware_interface: HardwareInterface = None):
        ...
        self.queue_full_log_interval = (
            30.0  # 每30秒最多记录一次队列满日志，减少日志噪音
        )
    ...
    def configure(self, config: AcquisitionConfig) -> bool:
        ...
        try:
            ...
            if success:
                self.printr(
                    f"采集配置成功: {len(config.channels)}通道, {config.sampling_rate}Hz"
                )
            else:
                self.printe("采集配置失败")
            return success
            ...
            try:
                ...
                if samples:
                    ...
                    try:
                        ...
                        if (
                            queue_usage >= self.queue_warning_threshold
                            and current_time - self.last_queue_warning_time
                            >= self.queue_warning_interval
                        ):
                            self.printr(
                                f"队列使用率过高: {queue_usage:.1%} ({queue_size}/{self.data_queue.maxsize})"
                            )
                        ...
                        current_time = time.time()
                        if (
                            current_time - self.last_queue_full_log_time
                            >= self.queue_full_log_interval
                        ):
    ...
    def _create_hardware_interface(self) -> HardwareInterface:
        """根据配置创建硬件接口"""
        if self.config.hardware_type == "real" or (
            self.config.hardware_type == "auto" and self.config.prefer_real_hardware
        ):
            # 尝试创建真实硬件接口
        ...
        while self._has_complete_set():
            ...
            for ch in self.channels:
                if self.channel_buffers[ch]:
                    if (
                        base_time is None
                        or self.channel_buffers[ch][0].timestamp < base_time
                    ):
