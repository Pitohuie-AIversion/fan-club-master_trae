    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if stats["samples_processed"] > 0:
            stats["avg_processing_time"] = (
                stats["processing_time_total"] / stats["samples_processed"]
            )
        return stats
...
class IIRFilter(DigitalFilter):
    ...
    def _design_bandpass(self, nyquist: float):
        """设计带通滤波器"""
        if (
            isinstance(self.config.cutoff_freq, list)
            and len(self.config.cutoff_freq) == 2
        ):
    ...
    def process_sample(self, sample: SampleData) -> FilteredData:
        ...
        return FilteredData(
            original_sample=sample, filtered_value=y, processing_time=processing_time
        )
...
class AdaptiveFilter(DigitalFilter):
    ...
    def process_sample(
        self, sample: SampleData, reference: float = None
    ) -> FilteredData:
        """处理单个样本（需要参考信号）"""
        if not self.is_initialized:
            self.initialize()
...
class KalmanFilter(DigitalFilter):
    ...
    def process_sample(self, sample: SampleData) -> FilteredData:
        ...
        self.statistics["kalman_gain_avg"] = (
            self.statistics["kalman_gain_avg"] * (n - 1) + self.K
        ) / n
        self.statistics["estimation_error_avg"] = (
            self.statistics["estimation_error_avg"] * (n - 1) + abs(y)
        ) / n
...
class AlphaBetaFilter(DigitalFilter):
    ...
    def process_sample(self, sample: SampleData) -> FilteredData:
        ...
        n = self.statistics["samples_processed"]
        if n > 1:
            self.statistics["velocity_avg"] = (
                self.statistics["velocity_avg"] * (n - 1) + abs(self.velocity)
            ) / n
...
class RealTimeFilterProcessor(pt.PrintClient):
    ...
    def add_filter(self, channel_id: int, filter_config: FilterConfig):
        ...
        try:
            ...
            self.printr(
                f"为通道{channel_id}添加{filter_config.filter_type.value}滤波器"
            )
            return True
                        ...
                        if channel_id in self.filters:
                            ...
                            for filter_instance in self.filters[channel_id]:
                                if filter_instance.config.enabled:
                                    filtered_data = filter_instance.process_sample(
                                        current_sample
                                    )
                                    # 为下一个滤波器创建新的样本
                            ...
                            filtered_samples.append(
                                FilteredData(
                                    original_sample=sample, filtered_value=sample.value
                                )
                            )
