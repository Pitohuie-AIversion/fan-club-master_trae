...
class SignalProcessingSystem:
    ...
    def initialize_components(self) -> bool:
        ...
        try:
            ...
            if self.config.filter_configs:
                ...
                self.logger.info(
                    f"数字滤波处理器初始化完成，加载了 {len(self.config.filter_configs)} 个滤波器"
                )
            ...
            self._trigger_event(
                "error_occurred", {"error": str(e), "component": "initialization"}
            )
            return False
    ...
    def start_system(self) -> bool:
        ...
        try:
            ...
            self._trigger_event(
                "error_occurred", {"error": str(e), "component": "system_start"}
            )
            return False
    ...
    def start_processing_threads(self):
        ...
        for i in range(self.config.processing_threads):
            thread = threading.Thread(
                target=self.data_processing_loop, name=f"DataProcessor-{i}", daemon=True
            )
            thread.start()
        ...
        acquisition_thread = threading.Thread(
            target=self.data_acquisition_loop, name="DataAcquisition", daemon=True
        )
    ...
    def start_monitoring_thread(self):
        ...
        self.monitoring_thread = threading.Thread(
            target=self.quality_monitoring_loop, name="QualityMonitor", daemon=True
        )
    ...
    def start_auto_save_thread(self):
        ...
        self.auto_save_thread = threading.Thread(
            target=self.auto_save_loop, name="AutoSave", daemon=True
        )
            ...
            try:
                ...
                if data is not None and len(data) > 0:
                    ...
                    try:
                        ...
                        self._trigger_event(
                            "data_acquired",
                            {"data_shape": data.shape, "timestamp": datetime.now()},
                        )
                ...
                self._trigger_event(
                    "error_occurred", {"error": str(e), "component": "data_acquisition"}
                )
    ...
    def data_processing_loop(self):
        ...
        while self.running:
            try:
                ...
                self._trigger_event(
                    "error_occurred", {"error": str(e), "component": "data_processing"}
                )
    ...
    def quality_monitoring_loop(self):
        ...
        while self.running:
            try:
                ...
                ) < self.config.quality_check_interval and self.running:
                    try:
                ...
                if data_for_analysis:
                    ...
                    for channel_idx in range(combined_data.shape[1]):
                        ...
                        # 质量分析
                        metrics = self.quality_monitor.analyze_quality(
                        ...
                        if metrics:
                            ...
                            self._trigger_event(
                                "quality_updated",
                                {"channel": channel_name, "metrics": metrics},
                            )

                        # 异常检测
                        anomalies = self.quality_monitor.detect_anomalies(
                            channel_data, channel_name
                        )
                        if anomalies:
                            self.statistics["total_anomalies_detected"] += len(
                                anomalies
                            )
                            ...
                            for anomaly in anomalies:
                                self._trigger_event(
                                    "anomaly_detected",
                                    {"channel": channel_name, "anomaly": anomaly},
                                )
                ...
                self._trigger_event(
                    "error_occurred",
                    {"error": str(e), "component": "quality_monitoring"},
                )
    ...
    def auto_save_loop(self):
        ...
        while self.running:
            try:
                ...
                self._trigger_event(
                    "error_occurred", {"error": str(e), "component": "auto_save"}
                )
    ...
    def manual_save_data(
        self, filename: str, format_type: DataFormat = DataFormat.HDF5
    ) -> Optional[str]:
        """手动保存数据"""
        try:
            # 收集当前缓冲区数据
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "thread_status": {
                ...
                "auto_save_thread_alive": (
                    self.auto_save_thread.is_alive() if self.auto_save_thread else False
                ),
            },
        }
...
if __name__ == "__main__":
    try:
        ...
        def on_quality_updated(data):
            ...
            print(
                f"质量更新 - {data['channel']}: 分数={metrics.quality_score:.1f}, 等级={metrics.quality_level.value}"
            )

        def on_anomaly_detected(data):
            anomaly = data["anomaly"]
            print(
                f"异常检测 - {data['channel']}: {anomaly.anomaly_type.value}, 严重程度={anomaly.severity:.2f}"
            )
