...
META = {
    ...
    broadcastIP: ("broadcastIP", 4, TYPE_PRIMITIVE, True, v_nonempty_str),
    broadcastPort: ("broadcastPort", 4, TYPE_PRIMITIVE, True, v_port),
    broadcastPeriodMS: ("broadcastPeriodMS", 4, TYPE_PRIMITIVE, True, v_positive_int),
    periodMS: ("periodMS", 4, TYPE_PRIMITIVE, True, v_positive_int),
    ...
    slaveQueueSize: ("slaveQueueSize", 4, TYPE_PRIMITIVE, True, v_positive_int),
    broadcastQueueSize: ("broadcastQueueSize", 4, TYPE_PRIMITIVE, True, v_positive_int),
    listenerQueueSize: ("listenerQueueSize", 4, TYPE_PRIMITIVE, True, v_positive_int),
    misoQueueSize: ("misoQueueSize", 4, TYPE_PRIMITIVE, True, v_positive_int),
    printerQueueSize: ("printerQueueSize", 4, TYPE_PRIMITIVE, True, v_positive_int),
    passcode: ("passcode", 4, TYPE_PRIMITIVE, True, v_nonempty_str),
    socketLimit: ("socketLimit", 4, TYPE_PRIMITIVE, True, v_positive_int),
    defaultIPAddress: ("defaultIPAddress", 4, TYPE_PRIMITIVE, True, v_nonempty_str),
    defaultBroadcastIP: ("defaultBroadcastIP", 4, TYPE_PRIMITIVE, True, v_nonempty_str),
    externalDefaultBroadcastIP: (
        "externalDefaultBroadcastIP",
        4,
        TYPE_PRIMITIVE,
        True,
    ...
    externalIndexDelta: ("externalIndexDelta", 4, TYPE_PRIMITIVE, True, v_positive_int),
    autoConnect: ("autoConnect", 4, TYPE_PRIMITIVE, True, v_bool),
    ...
    SV_index: ("SV_index", 2, TYPE_PRIMITIVE, True, v_negative_one),
    SV_fanModel: ("SV_fanModel", 3, TYPE_PRIMITIVE, True, v_str),
    SV_fanMode: ("SV_fanMode", 4, TYPE_PRIMITIVE, True, make_in_validator(FAN_MODES)),
    SV_targetRelation: (
        "SV_targetRelation",
        5,
        TYPE_PRIMITIVE,
        True,
        make_lambda_validator(
            lambda v: len(v) == 2
            and type(v[0]) in (float, int)
            and type(v[1]) in (float, int)
        ),
    ),
    SV_chaserTolerance: ("SV_chaserTolerance", 6, TYPE_PRIMITIVE, True, v_normalized),
    SV_fanFrequencyHZ: ("SV_fanFrequencyHZ", 7, TYPE_PRIMITIVE, True, v_positive_int),
    SV_counterCounts: ("SV_counterCounts", 8, TYPE_PRIMITIVE, True, v_positive_int),
    SV_counterTimeoutMS: (
        "SV_counterTimeoutMS",
        9,
        TYPE_PRIMITIVE,
    ...
    SV_maxRPM: ("SV_maxRPM", 11, TYPE_PRIMITIVE, True, v_positive_int),
    SV_minRPM: ("SV_minRPM", 12, TYPE_PRIMITIVE, True, v_nonnegative_int),
    SV_minDC: ("SV_minDC", 13, TYPE_PRIMITIVE, True, v_dutycycle),
    ...
    MD_assigned: ("MD_assigned", 16, TYPE_PRIMITIVE, True, v_bool),
    ...
    savedSlaves: ("savedSlaves", 6, TYPE_LIST, False, v_fail_all),
    pinouts: ("pinouts", 7, TYPE_MAP, False, v_pass_all),
    maxRPM: ("maxRPM", 8, TYPE_PRIMITIVE, True, v_positive_int),
    maxFans: ("maxFans", 8, TYPE_PRIMITIVE, True, v_positive_int),
    dcDecimals: ("dcDecimals", 8, TYPE_PRIMITIVE, True, v_nonnegative_int),
...
DEFAULTS = {savedSlaves: (defaultSlave, unpack_default_slave, index_slave)}
...
class FCArchive(pt.PrintClient):
            ...
            if key in META:
                ...
                try:
                    if not validator(value):
                        ...
                        errors.append(
                            f"Profile '{profile_name}' 中字段 '{field_name}' 验证失败"
                        )
                except Exception as e:
                    field_name = meta_info[NAME]
                    errors.append(
                        f"Profile '{profile_name}' 中字段 '{field_name}' 验证出错: {str(e)}"
                    )
                    ...
                    try:
                        if not validator(value):
                            ...
                            result["warnings"].append(
                                f"字段 '{field_name}' 可能存在问题"
                            )
                    except Exception as e:
                        field_name = meta_info[NAME]
                        result["errors"].append(
                            f"字段 '{field_name}' 验证失败: {str(e)}"
                        )
                        result["valid"] = False
        ...
        for field in port_fields:
            if field in self.P:
                ...
                if not (1 <= port_value <= 65535):
                    ...
                    result["errors"].append(
                        f"端口 '{field_name}' 超出有效范围 (1-65535): {port_value}"
                    )
                    result["valid"] = False
        ...
        for field in time_fields:
            if field in self.P:
                ...
                if time_value <= 0:
                    field_name = META[field][NAME]
                    result["errors"].append(
                        f"时间间隔 '{field_name}' 必须大于0: {time_value}"
                    )
                    result["valid"] = False
                elif time_value < 10:
                    field_name = META[field][NAME]
                    result["warnings"].append(
                        f"时间间隔 '{field_name}' 可能过小: {time_value}ms"
                    )
        ...
        for field in queue_fields:
            if field in self.P:
                ...
                if queue_size > 1000:
                    field_name = META[field][NAME]
                    result["warnings"].append(
                        f"队列大小 '{field_name}' 可能过大: {queue_size}"
                    )
    ...
    def restore_from_backup(self, backup_path):
        ...
        try:
            ...
            try:
                ...
                resolved_path = os.path.abspath(os.path.realpath(backup_path))
                current_dir = os.getcwd()
                if not resolved_path.startswith(current_dir):
                    raise IOError("备份文件路径必须在当前工作目录内")

                # 检查文件扩展名
                allowed_extensions = [".fc", ".backup", ".cfg"]
                file_ext = os.path.splitext(backup_path)[1].lower()
                if file_ext not in allowed_extensions:
                    raise IOError(f"不允许的备份文件类型: {file_ext}")
    ...
    def safe_set_value(self, key, value, create_backup=True):
        ...
        Returns:
            dict: 操作结果
        """
        result = {"success": False, "message": "", "backup_path": None}
        ...
        try:
            ...
            if not validation_result["valid"]:
                ...
                if old_value is not None:
                    ...
                    del self.P[key]
                result["message"] = (
                    f"设置值导致配置不一致，已回滚: {validation_result['errors']}"
                )
                return result
                ...
                if file_path:
                    for encoding in ENCODING_FALLBACKS:
                        try:
                            ...
                            self.printd(f"编码 {encoding} 失败: {e}")
                            continue
    ...
    def add(self, attribute, value):
        ...
        if self.meta[attribute][TYPE] is not TYPE_LIST:
            raise ValueError(
                "Tried to add to non-list attribute {} ".format(
        ...
        try:
            ...
            try:
                ...
                with open(name, "rb") as f:
                    ...
                    resolved_path = os.path.abspath(os.path.realpath(name))
                    if not resolved_path.startswith(os.getcwd()):
                        raise IOError("File path outside working directory not allowed")
    ...
    def __getitem__(self, key):
        ...
        try:
            ...
            self.printw(
                "Missing key {} loaded from default profile.".format(META[key][NAME])
            )
