...
class Array21x21Mapper:
    ...
    def get_neighbors(self, fan_index: int, radius: int = 1) -> List[int]:
        ...
        for row in range(
            max(0, center_row - radius), min(self.rows, center_row + radius + 1)
        ):
            for col in range(
                max(0, center_col - radius), min(self.columns, center_col + radius + 1)
            ):
    ...
    def create_circular_pattern(
        self, center_row: int, center_col: int, radius: float
    ) -> List[int]:
        """
...
def demo_usage():
    ...
    print(
        f"Module 10 controls fans: {mapper.get_fans_for_module(10)[:5]}... (showing first 5)"
    )
