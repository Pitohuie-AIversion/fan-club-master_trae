...
class TachSignalSimulator:
    ...
    def generate_pulse_train(self, rpm: int, duration_s: float) -> List[float]:
        ...
        pulse_interval = (
            1.0 / pulses_per_second if pulses_per_second > 0 else float("inf")
        )
    ...
    def measure_rpm_from_pulses(self, pulses: List[float]) -> TachReading:
        ...
        if measurement_time_us > 0:
            rpm = int(
                (60e6 * self.config.counter_counts)
...
class TachSignalAnalyzer:
    ...
    def get_latest_readings(self, count: int = 10) -> List[TachReading]:
        """获取最新读数"""
        return (
            list(self.readings)[-count:]
            if len(self.readings) >= count
            else list(self.readings)
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "signal_quality": {"mean": np.mean(qualities), "min": np.min(qualities)},
...
def main():
    ...
    try:
        ...
        if choice == "1":
            ...
            while True:
                try:
                    ...
                    if latest:
                        print(
                            f"最新RPM: {latest[-1].rpm}, 质量: {latest[-1].signal_quality:.3f}"
                        )
        ...
        if stats:
            ...
            print(
                f"平均RPM: {stats['rpm_stats']['mean']:.1f} ± {stats['rpm_stats']['std']:.1f}"
            )
            print(
                f"RPM范围: {stats['rpm_stats']['min']:.0f} - {stats['rpm_stats']['max']:.0f}"
            )
