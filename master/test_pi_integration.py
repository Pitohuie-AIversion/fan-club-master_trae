...
class PIIntegrationTester:
    ...
    def test_fccommunicator_piset_method(self):
        ...
        try:
            ...
            if has_method:
                self.log_result(
                    "FCCommunicator.sendPISet方法存在", True, "方法已正确添加"
                )
            else:
        ...
        for kp, ki, expected_valid, description in test_cases:
            try:
                ...
                if is_valid == expected_valid:
                    self.log_result(
                        f"参数验证 - {description}", True, f"kp={kp}, ki={ki}"
                    )
                else:
                    self.log_result(
                        f"参数验证 - {description}",
        ...
        for fan_id, kp, ki, expected_format in test_cases:
            try:
                ...
                if expected_format in actual_format:
                    self.log_result(
                        f"PISET命令格式 - 风扇{fan_id}", True, actual_format
                    )
                else:
                    self.log_result(
                        f"PISET命令格式 - 风扇{fan_id}",
                        False,
                ...
                self.log_result(
                    f"PISET命令格式 - 风扇{fan_id}", False, f"格式化异常: {e}"
                )
